...
export function Findings() {
  ...
  // Listen for findings from scanner events + actively poll
  useEffect(() => {
    ...
    (async () => {
      ...
      // Active polling: fetch findings from all scanner scans
      const pollFindings = async () => {
        try {
      ...
      pollFindings(); // Initial fetch
      pollTimer = setInterval(pollFindings, 5000);
    })();

    return () => { unlisten?.(); if (pollTimer) clearInterval(pollTimer); };
  }, []);
