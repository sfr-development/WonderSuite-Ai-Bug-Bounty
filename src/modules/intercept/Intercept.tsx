...
// ── Attack Registry ──
// Every attack is a data object — same attacks used by table, auto-scan, and MCP
interface AttackDef {
  id: string;
  name: string;
...
function AttackTablePanel(props: AttackTablePanelProps) {
  ...
  // Sort: detected first, then by risk
  ...
  function applyAttack(id: string) {
    // Dispatch to specific attack logic
    switch (id) {
      case 'emailSwap': {
      ...
      case 'jsonArrayInjection': {
        ...
        // JSON body — convert email fields to arrays
        if (body.startsWith('{')) {
          try {
          ...
          // Form body — duplicate email param as array notation
...
export function Intercept() {
  ...
  // Analyzes the current request and returns which attacks are applicable
        ...
        unlisten = await listen<any>('proxy-event', (event) => {
          ...
          if (data.type === 'intercept') {
            ...
            if (item.is_response && item.raw_response) {
              // This is a response interception — update the existing request's response
              setQueue((q) => {
              ...
              // Auto-populate response viewer if this matches current
              setResponseRaw(item.raw_response);
            } else {
              setQueue((q) => [...q, req]);
            }
          } else if (data.type === 'intercept_resolved') {
            // When a request is resolved, capture its response if available
            ...
            // Capture response from traffic events for response viewer
  ...
  const forward = useCallback(async () => {
    ...
    try {
      ...
      // Save the forwarded request so response tab stays visible
