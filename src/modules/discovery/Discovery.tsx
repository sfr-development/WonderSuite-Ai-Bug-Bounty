...
export function Discovery() {
  ...
  const startContentDiscovery = useCallback(async () => {
    ...
    try {
      ...
      // Use the backend scanner for content discovery
      // Attempt backend scanner (may not be available for all targets)
      try {
  ...
  const startSubdomainScan = useCallback(async () => {
    ...
    try {
      const { invoke } = await import('@tauri-apps/api/core');
      // Use common subdomain prefixes and DNS resolution
      const prefixes = [
