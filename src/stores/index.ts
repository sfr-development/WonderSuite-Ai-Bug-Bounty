...
interface AppState {
  ...
  appearance: { theme: string; accentColor: string; uiScale: number; compactMode: boolean; switchTabOnSend: boolean };
  updateAppearance: (updates: Partial<{ theme: string; accentColor: string; uiScale: number; compactMode: boolean; switchTabOnSend: boolean }>) => void;
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  sendTo: (tool, method, url, requestRaw, responseRaw, target) => {
    ...
    const shouldSwitch = get().appearance.switchTabOnSend !== false;

    if (shouldSwitch) {
      set({ activeModule, pendingSendTo: { tool, method, url, requestRaw, responseRaw, target } });
    } else {
      set({ pendingSendTo: { tool, method, url, requestRaw, responseRaw, target } });
      const toolName = tool.charAt(0).toUpperCase() + tool.slice(1);
      get().addToast({
        title: `Sent to ${toolName}`,
        message: `Request successfully sent to ${toolName} in the background.`,
        type: 'success',
      });
    }
  },
  ...
  appearance: {
    ...
    uiScale: 100,
    compactMode: false,
    switchTabOnSend: true,
    ...JSON.parse(localStorage.getItem('ws_appearance') || '{}'),
  },
