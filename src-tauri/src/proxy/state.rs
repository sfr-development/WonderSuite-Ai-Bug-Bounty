...
impl ProxyState {
    ...
    fn default_interception_rules() -> Vec<InterceptionRule> {
        vec![
            // By default, don't intercept static assets
            InterceptionRule {
                id: "skip-images".into(),
                enabled: true,
                name: "Skip images".into(),
                rule_type: InterceptionRuleType::FileExtension {
                    extensions: vec!["png".into(), "jpg".into(), "jpeg".into(), "gif".into(), "svg".into(), "ico".into(), "webp".into()],
                },
            ...
            InterceptionRule {
                ...
                rule_type: InterceptionRuleType::FileExtension {
                    extensions: vec!["css".into(), "js".into(), "woff".into(), "woff2".into(), "ttf".into(), "eot".into()],
                },
                target: InterceptionTarget::Both,
                action: "passthrough".into(),
            },
    ...
    pub async fn should_intercept_request(&self, method: &str, url: &str, host: &str, headers: &str) -> bool {
        ...
        for rule in rules.iter() {
            ...
            if !matches!(rule.target, InterceptionTarget::Request | InterceptionTarget::Both) { continue; }
        ...
        // Default: intercept if intercept is enabled globally
        true
    }
    ...
    pub async fn should_intercept_response(&self, url: &str, host: &str, status: u16, headers: &str) -> bool {
        ...
        for rule in rules.iter() {
            if !rule.enabled { continue; }
            if !matches!(rule.target, InterceptionTarget::Response | InterceptionTarget::Both) { continue; }
    ...
    fn rule_matches(&self, rule_type: &InterceptionRuleType, method: &str, url: &str, host: &str, headers: &str, status: Option<u16>) -> bool {
        match rule_type {
            ...
            InterceptionRuleType::HeaderContains { header, value } => {
                headers.lines().any(|l| {
            ...
            InterceptionRuleType::MimeType { pattern } => {
                headers.lines().any(|l| {
                    if let Some((k, v)) = l.split_once(':') {
    ...
    pub async fn apply_match_replace_request(&self, headers: &mut String, body: &mut String, url: &mut String) {
        let rules = self.match_replace_rules.read().await;
        for rule in rules.iter() {
            if !rule.enabled { continue; }
            if rule.direction != "request" && rule.direction != "both" { continue; }

            match rule.target.as_str() {
                "request_header" => {
                    *headers = self.apply_replacement(headers, &rule.match_pattern, &rule.replace_value, rule.is_regex);
                }
                "request_body" => {
                    *body = self.apply_replacement(body, &rule.match_pattern, &rule.replace_value, rule.is_regex);
                }
                "request_url" => {
                    *url = self.apply_replacement(url, &rule.match_pattern, &rule.replace_value, rule.is_regex);
                }
                _ => {}
            }
        }
    ...
    pub async fn apply_match_replace_response(&self, headers: &mut String, body: &mut Vec<u8>) {
        ...
        for rule in rules.iter() {
            if !rule.enabled { continue; }
            if rule.direction != "response" && rule.direction != "both" { continue; }
            ...
            match rule.target.as_str() {
                ...
                "response_body" => {
                    ...
                    let new_body = self.apply_replacement(&body_str, &rule.match_pattern, &rule.replace_value, rule.is_regex);
