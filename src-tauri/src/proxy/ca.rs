...
use dashmap::DashMap;
use der::asn1::{Ia5String, PrintableStringRef, Utf8StringRef};
...
impl ProxyCa {
    ...
    pub fn new() -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
        ...
        let (ca_cert_pem, ca_key_pem, ca_cert_der, ca_key) = if ca_cert_path.exists() && ca_key_path.exists()
        {
            ...
            let result = Self::generate_and_save(&ca_cert_path, &ca_key_path)?;
            println!("[CA] ✓ Generated new root CA (RSA-2048, pure Rust): {}", ca_cert_path.display());
            result
        };
    ...
    fn load_existing(
        ...
        let key =
            RsaPrivateKey::from_pkcs8_pem(&key_pem).map_err(|e| format!("Failed to parse CA key: {}", e))?;
    ...
    fn generate_ca(
        ...
        let ca_key =
            RsaPrivateKey::new(&mut rng, 2048).map_err(|e| format!("RSA key generation failed: {}", e))?;
        ...
        let builder = CertificateBuilder::new(Profile::Root, serial, validity, subject, ca_spki, &signer)
            .map_err(|e| format!("Certificate builder failed: {}", e))?;

        let cert = builder.build().map_err(|e| format!("Certificate build failed: {}", e))?;

        let cert_pem =
            cert.to_pem(der::pem::LineEnding::LF).map_err(|e| format!("PEM encoding failed: {}", e))?;
        let cert_der = cert.to_der().map_err(|e| format!("DER encoding failed: {}", e))?;
        let key_pem = ca_key
    ...
    pub fn generate_identity(
        ...
        self.identity_cache.insert(host.to_string(), Arc::new(CachedIdentity { pfx_data }));
    ...
    fn issue_host_cert(&self, host: &str) -> Result<Vec<u8>, Box<dyn std::error::Error + Send + Sync>> {
        let mut rng = rand::thread_rng();
        let host_key =
            RsaPrivateKey::new(&mut rng, 2048).map_err(|e| format!("Host key generation failed: {}", e))?;

        let host_pub = RsaPublicKey::from(&host_key);
        let host_spki =
            SubjectPublicKeyInfoOwned::from_key(host_pub).map_err(|e| format!("Host SPKI failed: {}", e))?;

        let serial_bytes: [u8; 16] = rand::random();
        let serial =
            SerialNumber::new(&serial_bytes).map_err(|e| format!("Serial creation failed: {}", e))?;

        let validity = Validity::from_now(std::time::Duration::from_secs(825 * 86400))
            .map_err(|e| format!("Validity creation failed: {}", e))?;

        let subject =
            Name::from_str(&format!("CN={}", host)).map_err(|e| format!("Subject name failed: {}", e))?;
        ...
        let mut builder = CertificateBuilder::new(
            Profile::Leaf { issuer, enable_key_agreement: false, enable_key_encipherment: true },
            serial,
            validity,
            subject,
            host_spki,
        ...
        let san = SubjectAltName(vec![GeneralName::DnsName(
            Ia5String::new(host).map_err(|e| format!("Invalid DNS name '{}': {}", host, e))?,
        )]);
        builder.add_extension(&san).map_err(|e| format!("SAN extension failed: {}", e))?;

        let host_cert = builder.build().map_err(|e| format!("Host cert build failed: {}", e))?;

        let host_cert_der = host_cert.to_der().map_err(|e| format!("Host cert DER failed: {}", e))?;
        let ca_cert_der = &self.ca_cert_der;

        let host_key_pkcs8_der =
            host_key.to_pkcs8_der().map_err(|e| format!("Host key DER failed: {}", e))?;
        ...
        for part in s.split(',') {
            ...
            if let Some((key, value)) = part.split_once('=') {
                ...
                let attr_value = if key.trim() == "C" || key.trim() == "c" {
                    ...
                    der::Any::from(
                        Utf8StringRef::new(value).map_err(|e| format!("Invalid value '{}': {}", value, e))?,
                    )
                };

                let atv = AttributeTypeAndValue { oid, value: attr_value };
