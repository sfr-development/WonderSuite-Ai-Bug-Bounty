...
impl ProxyEngine {
    ...
    async fn handle_connection(
        ...
        // Read initial request line with timeout
        ...
        if method == "CONNECT" {
            // For CONNECT: client waits for 200 before sending TLS data,
            // so no buffered data beyond headers → safe to recover inner stream.
            ...
            self.handle_http_request(
                &mut stream, &method, &full_url, &headers_raw, &header_pairs, &body, false,
            ).await
        }
    }
    ...
    async fn handle_connect(
        ...
        // Generate TLS identity for this host (RSA-2048, CA-signed)
        ...
        // Build TLS acceptor with the per-host cert
        ...
        // Split TLS stream for reading/writing
        ...
        loop {
            ...
            let should_continue = self.process_tunneled_request(
                &mut writer, &host, port, request,
            ).await;
    ...
    async fn process_tunneled_request<W: AsyncWriteExt + Unpin>(
        ...
        // Rebuild body if M&R changed it
        if body_str != String::from_utf8_lossy(&body) {
            body = body_str.as_bytes().to_vec();
        }

        // Build raw request for logging/intercept
        let raw_request = format!(
            "{} {} HTTP/1.1\r\n{}{}",
            method, path, headers_raw,
        ...
        match result {
            Ok((status, resp_headers, resp_body)) => {
                self.process_response(
                    ...
                    &final_method, &final_url, host, &path, port,
                    true, elapsed, &final_headers_raw, &final_body_str, &raw_request,
                ).await?;
            }
            Err(e) => {
        ...
        let conn_close = request.headers.iter().any(|(k, v)| {
    ...
    async fn handle_http_request(
        ...
        if self.state.should_intercept_request(&method, &url, &host, &headers_str).await {
            ...
            let item = InterceptedItem {
                ...
                raw_request,
                timestamp: chrono::Utc::now().to_rfc3339(),
                is_response: false, status: None, raw_response: None,
            };
            let (tx, rx) = oneshot::channel();
            self.state.add_intercept(PendingIntercept { item, sender: tx }).await;
        ...
        let header_map: Vec<(String, String)> = headers_str.lines()
        ...
        match result {
            Ok((status, resp_headers, resp_body)) => {
                self.process_response(
                    stream, status, resp_headers, resp_body,
                    &method, &url, &host, &path, port, is_tls, elapsed,
                    &headers_str, &body_str, &raw_request_log,
                ).await?;
            }
            Err(e) => {
    ...
    async fn process_response<W: AsyncWriteExt + Unpin>(
        ...
        let mut final_headers: Vec<(String, String)> = resp_headers_str.lines()
            .filter_map(|l| l.split_once(':').map(|(k, v)| (k.trim().to_string(), v.trim().to_string())))
            .collect();
        ...
        if self.state.should_intercept_response(url, host, status, &resp_headers_str).await {
            let resp_item = InterceptedItem {
                id: uuid::Uuid::new_v4().to_string(),
                method: method.to_string(), url: url.to_string(), host: host.to_string(),
                ...
                Ok(InterceptDecision::Forward(modified)) => {
                    if !modified.is_empty() {
                        if let Some((_s, new_headers, new_body)) = parse_modified_response(&modified) {
                            ...
                            resp_headers_str = final_headers.iter()
                                .map(|(k, v)| format!("{}: {}", k, v))
                                .collect::<Vec<_>>().join("\r\n");
                        }
                    }
        ...
        // Log traffic (cap stored response body to prevent memory bloat)
        let mime = final_headers.iter()
    ...
    async fn forward_request(
        ...
        for (k, v) in headers {
            let lower = k.to_lowercase();
            if HOP_BY_HOP_HEADERS.contains(&lower.as_str()) { continue; }
...
async fn read_wire_request<R: AsyncBufRead + Unpin>(
    ...
    if request_line.is_empty() { return Ok(None); }
    ...
    loop {
        ...
        if let Some((key, value)) = trimmed.split_once(':') {
            ...
            if lower == "content-length" { content_length = v.parse().unwrap_or(0); }
            if lower == "transfer-encoding" && v.to_lowercase().contains("chunked") { is_chunked = true; }
            if lower == "upgrade" && v.to_lowercase().contains("websocket") { is_websocket_upgrade = true; }
    ...
    Ok(Some(WireRequest {
        method, path, headers, headers_raw, body, is_websocket_upgrade,
    }))
}
...
fn parse_modified_response(raw: &str) -> Option<(u16, Vec<(String, String)>, String)> {
    let mut lines = raw.lines();
    let first = lines.next()?.trim();
    let status = first.split_whitespace()
