...
pub async fn proxy_status(state: tauri::State<'_, ProxyAppState>) -> Result<serde_json::Value, String> {
    ...
    let ca_path = ca_guard.as_ref().map(|c| c.ca_cert_path().to_string_lossy().to_string()).unwrap_or_default();
...
#[tauri::command]
pub async fn proxy_search_traffic(query: String, state: tauri::State<'_, ProxyAppState>) -> Result<Vec<TrafficEntry>, String> {
...
#[tauri::command]
pub async fn proxy_get_pending(state: tauri::State<'_, ProxyAppState>) -> Result<Vec<InterceptedItem>, String> {
...
// ─── Match & Replace Commands ────────────────────────────────────────────────

#[tauri::command]
pub async fn proxy_get_match_replace_rules(state: tauri::State<'_, ProxyAppState>) -> Result<Vec<MatchReplaceRule>, String> {
    Ok(state.proxy_state.match_replace_rules.read().await.clone())
}

#[tauri::command]
pub async fn proxy_add_match_replace_rule(rule: MatchReplaceRule, state: tauri::State<'_, ProxyAppState>) -> Result<String, String> {
...
#[tauri::command]
pub async fn proxy_update_match_replace_rule(rule: MatchReplaceRule, state: tauri::State<'_, ProxyAppState>) -> Result<bool, String> {
    let mut rules = state.proxy_state.match_replace_rules.write().await;
...
#[tauri::command]
pub async fn proxy_remove_match_replace_rule(id: String, state: tauri::State<'_, ProxyAppState>) -> Result<bool, String> {
    let mut rules = state.proxy_state.match_replace_rules.write().await;
...
// ─── Interception Rules Commands ─────────────────────────────────────────────

#[tauri::command]
pub async fn proxy_get_interception_rules(state: tauri::State<'_, ProxyAppState>) -> Result<Vec<InterceptionRule>, String> {
    Ok(state.proxy_state.interception_rules.read().await.clone())
}

#[tauri::command]
pub async fn proxy_add_interception_rule(rule: InterceptionRule, state: tauri::State<'_, ProxyAppState>) -> Result<String, String> {
...
#[tauri::command]
pub async fn proxy_update_interception_rule(rule: InterceptionRule, state: tauri::State<'_, ProxyAppState>) -> Result<bool, String> {
    let mut rules = state.proxy_state.interception_rules.write().await;
    if let Some(existing) = rules.iter_mut().find(|r| r.id == rule.id) {
        *existing = rule;
        Ok(true)
...
#[tauri::command]
pub async fn proxy_remove_interception_rule(id: String, state: tauri::State<'_, ProxyAppState>) -> Result<bool, String> {
    let mut rules = state.proxy_state.interception_rules.write().await;
    let len_before = rules.len();
    rules.retain(|r| r.id != id);
    Ok(rules.len() < len_before)
...
// ─── TLS Pass Through Commands ───────────────────────────────────────────────

#[tauri::command]
pub async fn proxy_get_tls_passthrough(state: tauri::State<'_, ProxyAppState>) -> Result<Vec<TlsPassThroughEntry>, String> {
    Ok(state.proxy_state.tls_passthrough.read().await.clone())
}

#[tauri::command]
pub async fn proxy_add_tls_passthrough(entry: TlsPassThroughEntry, state: tauri::State<'_, ProxyAppState>) -> Result<String, String> {
...
#[tauri::command]
pub async fn proxy_remove_tls_passthrough(id: String, state: tauri::State<'_, ProxyAppState>) -> Result<bool, String> {
    let mut list = state.proxy_state.tls_passthrough.write().await;
...
// ─── Upstream Proxy Commands ─────────────────────────────────────────────────

#[tauri::command]
pub async fn proxy_get_upstream(state: tauri::State<'_, ProxyAppState>) -> Result<UpstreamProxyConfig, String> {
    Ok(state.proxy_state.upstream_proxy.read().await.clone())
}

#[tauri::command]
pub async fn proxy_set_upstream(config: UpstreamProxyConfig, state: tauri::State<'_, ProxyAppState>) -> Result<bool, String> {
...
#[tauri::command]
pub async fn proxy_get_websocket_messages(state: tauri::State<'_, ProxyAppState>) -> Result<Vec<WebSocketMessage>, String> {
...
#[tauri::command]
pub async fn proxy_get_listeners(state: tauri::State<'_, ProxyAppState>) -> Result<Vec<ProxyListener>, String> {
    Ok(state.proxy_state.listeners.read().await.clone())
}

#[tauri::command]
pub async fn proxy_add_listener(listener: ProxyListener, state: tauri::State<'_, ProxyAppState>) -> Result<String, String> {
...
#[tauri::command]
pub async fn proxy_remove_listener(id: String, state: tauri::State<'_, ProxyAppState>) -> Result<bool, String> {
    let mut listeners = state.proxy_state.listeners.write().await;
...
#[tauri::command]
pub async fn proxy_import_ca_key(cert_path: String, key_path: String, state: tauri::State<'_, ProxyAppState>) -> Result<String, String> {
    // Verify files exist
    ...
    // For now, store the paths. The CA module would need to be extended to support loading custom keys.
    Ok(format!("CA key pair registered: cert={}, key={}", cert_path, key_path))
}

// ─── Capabilities & Statistics Commands ──────────────────────────────────────
...
#[tauri::command]
pub async fn proxy_get_statistics(state: tauri::State<'_, ProxyAppState>) -> Result<serde_json::Value, String> {
    let traffic = state.proxy_state.traffic.lock().await;
    ...
    let methods: std::collections::HashMap<String, usize> = {
        ...
        for t in traffic.iter() { *m.entry(t.method.clone()).or_insert(0) += 1; }
        m
    };
    ...
    let avg_response_time = if total > 0 {
        traffic.iter().map(|t| t.response_time_ms).sum::<u64>() / total as u64
    } else { 0 };
