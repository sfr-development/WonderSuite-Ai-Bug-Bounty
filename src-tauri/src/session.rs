...
impl CookieJar {
    ...
    pub fn get(&self, domain: &str, path: &str) -> Vec<&Cookie> {
        self.cookies.iter().filter(|c| {
            (domain.ends_with(&c.domain) || c.domain == domain) &&
        ...
        if let Some(main) = parts.first() {
            ...
            if kv.len() == 2 {
                ...
                for attr in &parts[1..] {
                    ...
                    else if attr.starts_with("path=") { cookie.path = attr[5..].to_string(); }
                    else if attr.starts_with("domain=") { cookie.domain = attr[7..].to_string(); }
                    else if attr.starts_with("samesite=") { cookie.samesite = Some(attr[9..].to_string()); }
                    else if attr.starts_with("expires=") { cookie.expires = Some(attr[8..].to_string()); }
                }
                self.set(cookie);
            }
        }
