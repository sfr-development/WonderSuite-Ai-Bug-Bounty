...
impl IntruderManager {
    pub fn new() -> Self { Self { attacks: HashMap::new() } }
}
...
// Generate payloads from a PayloadSet
    ...
    let mut raw = match set.payload_type.as_str() {
        ...
        "bruteforce" => {
            ...
            for len in min..=max {
                ...
                if results.len() > 10000 { break; } // Safety limit
            }
            results
        }
...
// Extract positions from request template
...
// Parse HTTP request from template into method + url + headers + body
fn parse_request_template(raw: &str) -> (String, String, Vec<(String, String)>, Option<String>) {
    ...
    for line in &lines[1..] {
        if let Some(idx) = line.find(':') {
            ...
            if key.to_lowercase() == "host" { host = val.clone(); }
...
pub async fn intruder_start(
    ...
    let payload_sets: Vec<Vec<String>> = config.payload_sets.iter()
        .map(|s| generate_payloads(s))
        .collect();
    ...
    let payload_matrix: Vec<Vec<String>> = match config.attack_type.as_str() {
        ...
        "pitchfork" => {
            ...
            (0..max_len).map(|i| {
                payload_sets.iter().map(|s| s.get(i).cloned().unwrap_or_default()).collect()
            }).collect()
        }
        "cluster_bomb" => {
            ...
            for set in &payload_sets {
                ...
                combos = new_combos;
                if combos.len() > 100000 { break; } // Safety
            }
            combos
        }
        _ => { // sniper
    ...
    // Initialize attack state
    {
        ...
        mgr.attacks.insert(aid.clone(), AttackState {
            id: aid.clone(),
            status: "running".into(),
        ...
        for (i, payloads) in payload_matrix.iter().enumerate() {
            ...
            match req.send().await {
                Ok(resp) => {
                    ...
                    result.response_headers = resp.headers().iter()
                    ...
                    for rule in &grep_rules {
                        match rule.rule_type.as_str() {
                            "match" => {
                                ...
                                } else if resp_body.contains(&rule.pattern) {
                                    result.grep_match = true;
                                }
                            }
                            "extract" => {
                                if let Ok(re) = regex::Regex::new(&rule.pattern) {
                                    if let Some(caps) = re.captures(&resp_body) {
                                        ...
                                        if let Some(m) = caps.get(group) {
                                            let name = rule.name.clone().unwrap_or_else(|| format!("extract_{}", group));
...
pub async fn intruder_results(
    ...
    let results: Vec<&AttackResult> = attack.results.iter()
        .filter(|r| r.id > from)
        .collect();
    Ok(serde_json::json!({
        "status": attack.status,
