...
pub async fn handle_raw_tcp_send(params: &serde_json::Value) -> HandlerResult {
    ...
    let tcp_stream =
        tokio::net::TcpStream::connect(&addr).await.map_err(|e| format!("TCP connect failed: {}", e))?;
    ...
    if use_tls {
        ...
        let mut tls_stream =
            cx.connect(host, tcp_stream).await.map_err(|e| format!("TLS handshake: {}", e))?;
        tls_stream.write_all(&raw).await.map_err(|e| format!("Write: {}", e))?;
        tls_stream.flush().await.ok();
        let mut buf = vec![0u8; read_size];
        let n =
            tokio::time::timeout(std::time::Duration::from_millis(read_timeout), tls_stream.read(&mut buf))
                .await
                .unwrap_or(Ok(0))
                .unwrap_or(0);
        Ok(
            serde_json::json!({"bytes_sent": raw.len(), "bytes_received": n, "connect_ms": connect_ms, "total_ms": start.elapsed().as_millis(), "tls": true, "response": String::from_utf8_lossy(&buf[..n]), "response_hex": buf[..n].iter().map(|b| format!("{:02x}", b)).collect::<Vec<_>>().join(" ")}),
        )
    } else {
        ...
        let mut buf = vec![0u8; read_size];
        let n = tokio::time::timeout(std::time::Duration::from_millis(read_timeout), stream.read(&mut buf))
            .await
            .unwrap_or(Ok(0))
            .unwrap_or(0);
        Ok(
            serde_json::json!({"bytes_sent": raw.len(), "bytes_received": n, "connect_ms": connect_ms, "total_ms": start.elapsed().as_millis(), "tls": false, "response": String::from_utf8_lossy(&buf[..n]), "response_hex": buf[..n].iter().map(|b| format!("{:02x}", b)).collect::<Vec<_>>().join(" ")}),
        )
    }
}
...
pub async fn handle_mtls_send_request(params: &serde_json::Value) -> HandlerResult {
    ...
    Ok(
        serde_json::json!({"status": status, "headers": resp_headers, "body_preview": body.chars().take(2000).collect::<String>(), "body_size": body.len(), "elapsed_ms": elapsed, "mtls": true}),
    )
}
    ...
    std::thread::spawn(move || {
        ...
        let result = rt.block_on(async move {
            ...
            let response_times: Vec<u64> =
                race_results.iter().filter_map(|r| r["response_ms"].as_u64()).collect();
...
pub async fn handle_bambda_filter(params: &serde_json::Value) -> HandlerResult {
    ...
    if let Some(traffic) = traffic_json {
        let filtered: Vec<&serde_json::Value> = traffic
            .iter()
            .filter(|item| crate::mcp::utils::evaluate_bambda_conditions(item, &conditions))
            .collect();
        Ok(
            serde_json::json!({"expression": expression, "total_items": traffic.len(), "matched_items": filtered.len(), "filtered": filtered, "conditions_parsed": conditions.len()}),
        )
    } else {
        Ok(
            serde_json::json!({"expression": expression, "valid": true, "conditions_parsed": conditions.len(), "conditions": conditions.iter().map(|c| serde_json::json!({"field": c.field, "operator": c.operator, "value": c.value})).collect::<Vec<_>>(), "note": "Expression parsed successfully. Provide 'traffic' array to apply the filter."}),
        )
    }
}
