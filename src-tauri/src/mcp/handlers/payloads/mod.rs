...
impl PayloadManager {
    pub fn new() -> Self {
        let base_dir =
            dirs::home_dir().unwrap_or_else(|| PathBuf::from(".")).join(".wondersuite").join("payloads");
        ...
        categories
            ...
            .map(|cat| {
                ...
                let file_count = if dir.exists() {
                    std::fs::read_dir(&dir).map(|rd| rd.filter(|e| e.is_ok()).count()).unwrap_or(0)
                } else {
                    0
                };
                let total_payloads = if dir.exists() { self.count_payloads_in_dir(&dir) } else { 0 };
                CategoryInfo {
                    name: cat.to_string(),
            ...
            if let Ok(payloads) = self.load(cat) {
                for payload in &payloads {
                    if payload.to_lowercase().contains(&query_lower) {
                        results.push(SearchResult { category: cat.to_string(), payload: payload.clone() });
                        if results.len() >= 100 {
                            return results;
                        }
                    }
...
pub async fn handle_payload_manager(params: &serde_json::Value) -> HandlerResult {
    ...
    match action {
        ...
        "download" => {
            ...
            let categories =
                if category == "all" { registry::all_categories() } else { vec![category.to_string()] };
        ...
        "load" => {
            let category = params["category"].as_str().ok_or("category is required for load action")?;
        ...
        _ => Err(format!(
            "Unknown payload_manager action: {}. Use: list, download, search, info, load",
            action
        )),
    }
}
