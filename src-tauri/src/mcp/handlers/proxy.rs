// ═══════════════════════════════════════════════════════════════════════
//  Proxy Handlers — REAL implementations that access ProxyState directly
//  via the global OnceLock bridge (no Tauri managed state needed)
// ═══════════════════════════════════════════════════════════════════════
...
pub async fn handle_proxy_start(params: &serde_json::Value) -> HandlerResult {
    ...
    *ps.event_tx.lock().await = Some(tx);
    ...
    // Store handle + cancel globally for MCP stop
...
pub async fn handle_proxy_stop(_params: &serde_json::Value) -> HandlerResult {
    ...
    // Drain pending intercepts to prevent hanging requests
    ps.drain_pending_intercepts().await;
    ps.set_intercept(false);
    ps.set_response_intercept(false);

    // Cancel the engine via global shutdown handle
    let shutdown = crate::proxy_commands::get_or_init_global_shutdown().await;
...
pub async fn handle_proxy_toggle_intercept(params: &serde_json::Value) -> HandlerResult {
    ...
    // When disabling, drain pending intercepts to prevent hanging
    if !enabled {
        ps.drain_pending_intercepts().await;
    }
...
pub async fn handle_proxy_search_traffic(params: &serde_json::Value) -> HandlerResult {
    ...
    let entries_json: Vec<serde_json::Value> = results.iter().take(200).map(|e| {
        serde_json::json!({
...
pub async fn handle_proxy_add_match_replace(params: &serde_json::Value) -> HandlerResult {
    ...
    let target = params["target"].as_str().ok_or("Missing target (request_header, request_body, response_header, response_body, request_url)")?;
...
pub async fn handle_proxy_set_upstream(params: &serde_json::Value) -> HandlerResult {
    ...
    let config = UpstreamProxyConfig {
        ...
        bypass_patterns: params["bypass_patterns"].as_array()
...
pub async fn handle_proxy_get_websocket_messages(_params: &serde_json::Value) -> HandlerResult {
    ...
    let msgs_json: Vec<serde_json::Value> = messages.iter().rev().take(200).map(|m| {
        serde_json::json!({
...
pub async fn handle_proxy_export_traffic(params: &serde_json::Value) -> HandlerResult {
    ...
    match format {
        ...
        "har" => {
            // Basic HAR 1.2 export
            let entries: Vec<serde_json::Value> = traffic.iter().map(|t| {
                serde_json::json!({
...
// ═════════════════════════════════════════════════════════════════════
//  PROXY MANAGEMENT — Rule removal, traffic annotation
// ═════════════════════════════════════════════════════════════════════
...
pub async fn handle_proxy_annotate_traffic(params: &serde_json::Value) -> HandlerResult {
    ...
    let entry = traffic.iter_mut().find(|e| e.id == traffic_id)
...
// ═════════════════════════════════════════════════════════════════════
//  WORKFLOW TOOLS — Send to Repeater / Intruder + Intercept Control
// ═════════════════════════════════════════════════════════════════════
...
pub async fn handle_send_to_repeater(params: &serde_json::Value) -> HandlerResult {
    ...
    // Build the request — either from traffic ID or from raw params
    let (method, url, headers, body) = if let Some(traffic_id) = params["traffic_id"].as_u64() {
        // Load from proxy traffic history
        let traffic = ps.traffic.lock().await;
        let entry = traffic.iter().find(|e| e.id == traffic_id)
            .ok_or(format!("Traffic entry {} not found", traffic_id))?;
        ...
        // Apply header overrides from JSON object
    ...
    match req.send().await {
        Ok(resp) => {
            ...
            // Log to traffic as "repeater" source
            let entry = TrafficEntry {
                id: ps.next_id(),
                timestamp: chrono::Utc::now().to_rfc3339(),
                method: method.clone(),
                url: url.clone(),
                host: url::Url::parse(&url).map(|u| u.host_str().unwrap_or("").to_string()).unwrap_or_default(),
                ...
                mime_type: resp_headers.iter().find(|(k, _)| k == "content-type").map(|(_, v)| v.clone()).unwrap_or_default(),
                request_headers: headers.iter().map(|(k, v)| format!("{}: {}", k, v)).collect::<Vec<_>>().join("\r\n"),
                request_body: body.clone(),
                response_headers: resp_headers.iter().map(|(k, v)| format!("{}: {}", k, v)).collect::<Vec<_>>().join("\r\n"),
...
pub async fn handle_send_to_intruder(params: &serde_json::Value) -> HandlerResult {
    ...
    let traffic_id = params["traffic_id"].as_u64()
        .ok_or("traffic_id is required")?;

    let traffic = ps.traffic.lock().await;
    let entry = traffic.iter().find(|e| e.id == traffic_id)
        .ok_or(format!("Traffic entry {} not found", traffic_id))?;

    // Parse URL to find query parameters
    ...
    // Parse headers
    let headers: std::collections::HashMap<String, String> = entry.request_headers
        .lines()
        .filter_map(|line| {
    ...
    // Build suggested positions — mark each query param as a §position§
    ...
    // Also suggest body positions for POST requests
    if entry.method == "POST" && !entry.request_body.is_empty() {
        // Try to detect JSON body params
    ...
    // Build the intruder config that can be passed directly to fuzz_request
...
pub async fn handle_get_intercepted(_params: &serde_json::Value) -> HandlerResult {
    ...
    let items: Vec<serde_json::Value> = pending.values().map(|p| {
        // Parse raw_request into structured components
                ...
                if let Some((k, v)) = pair.split_once('=') {
                    query_params.insert(
                        ...
                        serde_json::Value::String(
                            v.replace("%20", " ").replace("%3D", "=").replace("%26", "&")
                             .replace("%3F", "?").replace("%2F", "/").replace("%3A", ":")
                             .replace("+", " ")
                        ),
                    );
                }
        ...
        let path = p.item.url.split('?').next().unwrap_or(&p.item.url)
...
pub async fn handle_forward_intercepted(params: &serde_json::Value) -> HandlerResult {
    ...
    match action {
        "forward" => {
            // Mode 1: Raw string modification (existing behavior)
            if let Some(raw) = params["modified_raw"].as_str() {
                if !raw.is_empty() {
                    let _ = item.sender.send(InterceptDecision::Forward(raw.to_string()));
                    drop(pending);
                    ps.emit(ProxyEvent::InterceptResolved { id: id.to_string(), action: "forward_modified_raw".to_string() }).await;
                    return Ok(serde_json::json!({
            ...
            // Mode 2: Structured JSON modification
            if let Some(modify) = params.get("modify") {
                ...
                if lines.is_empty() {
                    ...
                    // Apply header overrides (replace entire header set)
                ...
                drop(pending);
                ps.emit(ProxyEvent::InterceptResolved { id: id.to_string(), action: "forward_modified_json".to_string() }).await;
                return Ok(serde_json::json!({
                    "id": id, "action": "forward", "mode": "structured_edit", "status": "resolved"
                }));
            }

            // Mode 0: Forward unmodified
            let _ = item.sender.send(InterceptDecision::Forward(String::new()));
        }
        "drop" => {
    ...
    // Wait briefly for the proxy engine to forward and log the response
    if action == "forward" {
        tokio::time::sleep(tokio::time::Duration::from_millis(500)).await;
        // Try to get the latest traffic entry with the response
        let traffic = ps.traffic.lock().await;
