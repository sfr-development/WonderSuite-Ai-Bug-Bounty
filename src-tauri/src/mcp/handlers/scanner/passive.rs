...
pub async fn handle_passive_scan(params: &serde_json::Value) -> HandlerResult {
    ...
    if check_all || check_filter.contains(&"headers".into()) {
        ...
        check_missing_header(
            ...
            "Permissions-Policy",
            "info",
            "Restricts browser features (camera, geolocation, etc.)",
                        ...
                        findings.push(Finding {
                            ...
                            evidence:
                                "Access-Control-Allow-Origin: *\nAccess-Control-Allow-Credentials: true"
                                    .into(),
                            detail: "Wildcard origin with credentials is a dangerous misconfiguration."
                                .into(),
                            remediation: "Remove Access-Control-Allow-Credentials or use specific origins."
                                .into(),
                            request_info: None,
                        });
                    }
...
fn check_cookie_flags(set_cookie: &str, target: &str, findings: &mut Vec<Finding>) {
    ...
    if !cookie_lower.contains("httponly") {
        findings.push(Finding {
            ...
            detail: "Without HttpOnly, JavaScript can access this cookie, enabling XSS-based session theft."
                .into(),
