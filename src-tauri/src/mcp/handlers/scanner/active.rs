...
//  Probes for: SQLi, XSS, SSTI, LFI, Open Redirect, Header Injection
//  Uses PayloadManager for dynamic payload loading
// ═══════════════════════════════════════════════════════════════════════
...
pub async fn handle_active_scan(params: &serde_json::Value) -> HandlerResult {
    ...
    let scan_types: Vec<String> = params["scan_types"].as_array()
    ...
    let params_list: Vec<(String, String)> = parsed.query_pairs()
    ...
    if scan_all || scan_types.iter().any(|s| s == "xss") {
        let mut stats = ScanTypeStats::default();
        let canary = format!("ws{}", uuid::Uuid::new_v4().to_string().replace('-', "").get(..8).unwrap_or("test1234"));

        // Reflection test first
        for (param_name, _) in &params_list {
            ...
            if let Ok(resp) = client.get(&test_url).send().await {
                ...
                if body.contains(&canary) {
                    // Input is reflected! Now test with actual XSS payloads
                    ...
                    let all_xss: Vec<String> = context_payloads.into_iter()
                        .chain(xss_payloads.into_iter())
                        .collect();
                    ...
                    for payload in &all_xss {
                        ...
                        if let Ok(resp) = client.get(&test_url).send().await {
                            ...
                            // Check for partial reflection (tag chars survive)
    ...
    if scan_all || scan_types.iter().any(|s| s == "open_redirect") {
        ...
        let redirect_params = ["url", "redirect", "next", "return", "returnUrl", "redirect_uri",
                               "continue", "dest", "destination", "go", "target", "rurl", "out"];
