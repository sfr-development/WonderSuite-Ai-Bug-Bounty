// ═══════════════════════════════════════════════════════════════════════
//  Fuzzer — Multi-mode request fuzzer (Sniper, Battering Ram, Pitchfork, Cluster Bomb)
//  Integrates with PayloadManager for dynamic payload loading
// ═══════════════════════════════════════════════════════════════════════
...
pub async fn handle_fuzz_request(params: &serde_json::Value) -> HandlerResult {
    ...
    let base_headers: HashMap<String, String> = base["headers"].as_object()
        .map(|o| o.iter().map(|(k, v)| (k.clone(), v.as_str().unwrap_or("").to_string())).collect())
        .unwrap_or_default();

    // Parse positions & payloads
    let positions = params["positions"].as_array()
        .ok_or("positions array is required")?;
    ...
    // Send baseline request (no substitution) for comparison
    ...
    // Execute all requests with semaphore for concurrency
...
fn resolve_payloads(pos: &serde_json::Value) -> Result<Vec<String>, String> {
    ...
    match source {
        ...
        "file" => {
            let category = pos["file_category"].as_str()
                .ok_or("file_category is required for source=file")?;
...
fn generate_combinations(
    ...
    match attack_type {
        "sniper" => {
            // One position at a time, others get empty string
            for (i, pos) in positions.iter().enumerate() {
                for payload in &pos.payloads {
                    ...
                    combos.push(combo);
                    if combos.len() >= max_requests { return Ok(combos); }
                }
            }
        }
        "battering_ram" | "batteringram" => {
            ...
            if let Some(first) = positions.first() {
                for payload in &first.payloads {
                    let combo: Vec<(String, String)> = positions.iter()
                        .map(|p| (p.marker.clone(), payload.clone()))
                        .collect();
                    combos.push(combo);
                    if combos.len() >= max_requests { return Ok(combos); }
                }
        ...
        "pitchfork" => {
            // Parallel lists, 1:1 index
            let min_len = positions.iter().map(|p| p.payloads.len()).min().unwrap_or(0);
            for i in 0..min_len {
                let combo: Vec<(String, String)> = positions.iter()
                    .map(|p| (p.marker.clone(), p.payloads[i].clone()))
                    .collect();
                combos.push(combo);
                if combos.len() >= max_requests { return Ok(combos); }
            }
        ...
        "cluster_bomb" | "clusterbomb" => {
            ...
            loop {
                let combo: Vec<(String, String)> = positions.iter().enumerate()
        ...
        _ => return Err(format!("Unknown attack type: {}. Use: sniper, battering_ram, pitchfork, cluster_bomb", attack_type)),
    }
            ...
            match rule_type {
                "status_code" => {
                    if let Some(vals) = rule["values"].as_array() {
                        let codes: Vec<u16> = vals.iter().filter_map(|v| v.as_u64().map(|n| n as u16)).collect();
...
fn check_matches(result: &ResponseInfo, baseline: &ResponseInfo, rules: &[MatchRule], length_diff: usize, time_ms: u64) -> Vec<String> {
...
async fn send_one(client: &reqwest::Client, method: &str, url: &str, headers: &HashMap<String, String>, body: &str) -> ResponseInfo {
    ...
    match req.send().await {
        ...
        Err(_) => ResponseInfo { status: 0, length: 0, body: String::new(), time_ms: start.elapsed().as_millis() as u64 },
    }
}
