// ═══════════════════════════════════════════════════════════════════════
//  OAST Infrastructure — callback server primitives for blind vuln detection
//  The AI decides WHAT to inject and WHERE. This provides the callback infra.
// ═══════════════════════════════════════════════════════════════════════
...
pub async fn handle_oast_poll_interactions(params: &serde_json::Value) -> HandlerResult {
    ...
    static OAST_INTERACTIONS: std::sync::LazyLock<std::sync::Mutex<Vec<serde_json::Value>>> = std::sync::LazyLock::new(|| std::sync::Mutex::new(Vec::new()));
    let interactions = OAST_INTERACTIONS.lock().unwrap_or_else(|e| e.into_inner()).clone();
    let filtered: Vec<&serde_json::Value> = if let Some(cid) = correlation_id { interactions.iter().filter(|i| i["correlation_id"].as_str() == Some(cid)).collect() } else { interactions.iter().collect() };
    Ok(serde_json::json!({"status": "polled", "total_interactions": filtered.len(), "interactions": filtered}))
}
...
pub async fn handle_oast_start_server(params: &serde_json::Value) -> HandlerResult {
    ...
    Ok(serde_json::json!({"status": "server_started", "http_port": http_port, "callback_url": format!("http://127.0.0.1:{}", http_port)}))
}
...
pub async fn handle_oast_verify(params: &serde_json::Value) -> HandlerResult {
    ...
    static OAST_LOG: std::sync::LazyLock<tokio::sync::Mutex<Vec<serde_json::Value>>> = std::sync::LazyLock::new(|| tokio::sync::Mutex::new(Vec::new()));
    static OAST_RUNNING: std::sync::LazyLock<std::sync::atomic::AtomicBool> = std::sync::LazyLock::new(|| std::sync::atomic::AtomicBool::new(false));
    ...
    match action {
        "start_server" | "self_test" => {
            if !OAST_RUNNING.load(std::sync::atomic::Ordering::Relaxed) {
                ...
                tokio::spawn(async move {
                    ...
                    if let Some(l) = tokio::net::TcpListener::bind(addr).await.ok() { axum::serve(l, app).await.ok(); }
                });
            ...
            if action == "self_test" {
                ...
                Ok(serde_json::json!({"action": "self_test", "server_running": true, "port": port, "callback_received": found, "status": if found { "WORKING" } else { "FAILED" }}))
            } else {
        ...
        "get_interactions" => {
            ...
            let filtered: Vec<&serde_json::Value> = if let Some(cid) = corr_id { log.iter().filter(|e| e["uri"].as_str().map_or(false, |u| u.contains(cid))).collect() } else { log.iter().collect() };
            Ok(serde_json::json!({"total": log.len(), "filtered": filtered.len(), "interactions": filtered}))
        }
        "clear" => { OAST_LOG.lock().await.clear(); Ok(serde_json::json!({"action": "clear", "status": "cleared"})) }
