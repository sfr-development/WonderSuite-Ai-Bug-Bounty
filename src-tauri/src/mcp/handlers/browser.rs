// ═══════════════════════════════════════════════════════════════════════
//  Browser Handlers — CDP navigation, JS execution, session extraction
// ═══════════════════════════════════════════════════════════════════════
    ...
    match action {
        "open" | "navigate" => {
            ...
            if !cdp_active {
                ...
                // Auto-start CDP network traffic capture in background
                let capture_port = cdp_port;
                tokio::spawn(async move {
                    // Wait a moment for CDP to stabilize
...
pub async fn handle_browser_execute_js(params: &serde_json::Value) -> HandlerResult {
    ...
    let tabs: Vec<serde_json::Value> = tabs_resp.json().await.map_err(|e| format!("Failed to parse CDP tabs: {}", e))?;
    ...
    ws_stream.send(Message::Text(eval_msg.to_string().into())).await.map_err(|e| format!("CDP send failed: {}", e))?;
            ...
            while let Some(msg) = ws_stream.next().await {
                if let Ok(Message::Text(ref text)) = msg {
                    if let Ok(resp) = serde_json::from_str::<serde_json::Value>(&text) {
                        if resp["id"].as_u64() == Some(1) { return Ok(resp); }
                    }
                }
            }
...
pub async fn handle_session_from_browser(params: &serde_json::Value) -> HandlerResult {
    ...
    let (mut ws, _) = tokio_tungstenite::connect_async(ws_url).await.map_err(|e| format!("CDP connect: {}", e))?;
...
// ═══════════════════════════════════════════════════════════════════════
//  Browser Network Traffic — Read captured CDP network events
//  Actions: get (list traffic), clear, status, filter
//  Provides hints when browser is not running or no traffic captured.
// ═══════════════════════════════════════════════════════════════════════
...
pub async fn handle_browser_network_traffic(params: &serde_json::Value) -> HandlerResult {
    ...
    match action {
        ...
        "get" | "filter" => {
            ...
            let filtered: Vec<&crate::browser::NetworkEntry> = all_entries.iter()
                .filter(|e| {
                    // URL filter
                    if let Some(f) = url_filter {
                        if !e.url.to_lowercase().contains(&f.to_lowercase()) { return false; }
                    }
                    // Method filter
                    ...
                    if let Some(s) = status_filter {
                        if e.status != Some(s) { return false; }
                    }
                    // Resource type filter
                    if let Some(t) = type_filter {
                        if e.resource_type.as_deref() != Some(t) { return false; }
                    }
                    // Exclude static assets
                    if exclude_static {
                        let rtype = e.resource_type.as_deref().unwrap_or("");
                        if matches!(rtype, "Image" | "Font" | "Media") { return false; }
                        // Also skip data: URLs and chrome-extension:
            ...
            // Build summary with interesting findings
            ...
            for e in &filtered {
                ...
                if url_lower.contains("auth") || url_lower.contains("login") || url_lower.contains("token")
                    || url_lower.contains("session") || url_lower.contains("identity") || url_lower.contains("oauth") {
        ...
        "start_capture" => {
            // Manual start — useful if browser was launched externally
            if !cdp_active {
