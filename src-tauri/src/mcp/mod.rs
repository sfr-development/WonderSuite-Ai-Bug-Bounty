// ═══════════════════════════════════════════════════════════════════════
//  MCP Module — Modular MCP Tool Architecture
// ═══════════════════════════════════════════════════════════════════════
//
//  This is the new modular entry point for the MCP subsystem.
//  During migration, this re-exports from the old monolithic mcp.rs
//  while new modules are built in parallel.
//
...
//    ├── state.rs        (shared pentest state)
//    ├── router.rs       (Axum HTTP handlers)
//    ├── activity.rs     (activity & traffic logging)
//    ├── definitions/    (tool schemas by category)
//    └── handlers/       (tool implementations by category)
//
// ═══════════════════════════════════════════════════════════════════════
...
// These maintain the same public API that lib.rs and commands.rs expect:
//   crate::mcp::McpState
//   crate::mcp::create_mcp_state()
//   crate::mcp::handle_tool_call()
//   crate::mcp::ActivityEntry
//   crate::mcp::McpTrafficEntry
//   crate::mcp::get_activity_log()
//   crate::mcp::get_activity_stats()
//   crate::mcp::get_mcp_traffic()
...
impl McpServer {
    ...
    pub fn start_sync(&mut self, port: u16) -> Result<(), String> {
                ...
                rt.block_on(async move {
                    let app = Router::new()
                        .route("/mcp", post(|body: axum::body::Bytes| async move {
                            router::handle_rpc(body).await
                        }).get(router::handle_mcp_get));

                    // Try ports: requested, +1, +2
                    ...
                    for &p in &ports {
                        ...
                        match tokio::net::TcpListener::bind(addr).await {
                            Ok(l) => { listener_opt = Some(l); bound = p; break; }
                            Err(e) => { println!("[MCP] Port {} unavailable: {}", p, e); }
                        }
                    }
        ...
        // Wait for the server to signal it's ready (max 5s)
