...
pub fn tool_definitions() -> Vec<ToolDef> {
    vec![
        ...
        ToolDef {
            ...
            description: "[lifecycle] Attach to a running **WonderBrowser** CDP session — only ever the bundled WonderBrowser. With no cdp_port, scans 9333 (default), 9222, 9223. If nothing is reachable AND auto_launch:true, spawns a fresh WonderBrowser exactly like browser_open (same proxy wiring, same isolated profile, same stealth extension).\n\nIMPORTANT: This tool refuses to drive system Chrome / Edge / Brave even if they happen to be listening on a CDP port — those are the user's daily-driver browsers and touching them risks the user's real cookies, extensions, and logged-in sessions. If you find a non-WonderBrowser CDP responder, you get code=NOT_WONDERBROWSER. In that case call browser_open instead.\n\nWhen to use this vs browser_open:\n  • browser_attach: an earlier `browser_open` is still alive (e.g. a previous tool call left the session running) and you want to reuse it without spawning a duplicate.\n  • browser_open: nothing is running and you want a fresh WonderBrowser. (You can also call browser_attach with auto_launch:true to get the same result — they're equivalent in that path.)".into(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "cdp_port": { "type": "integer", "description": "Specific port to attach to. Omit to scan 9333/9222/9223." },
                    "proxy_port": { "type": "integer", "default": 8080 },
                    "url": { "type": "string", "description": "Optional URL to navigate to after attaching." },
                    "auto_launch": { "type": "boolean", "default": false, "description": "If no WonderBrowser is reachable, spawn one (uses the same code path as browser_open)." }
                }
            }),
        },
        ToolDef {
