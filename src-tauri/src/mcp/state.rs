// ═══════════════════════════════════════════════════════════════════════
//  MCP Shared Pentest State — AI Context for Exploit Chain Building
// ═══════════════════════════════════════════════════════════════════════
//
//  This is the KEY differentiator: every tool call can read and write
//  to this shared state, enabling the AI to:
//  - Chain tool outputs → inputs automatically
//  - Track discovered endpoints, sessions, OAST correlations
//  - Build exploit chains without losing context between calls
//
// ═══════════════════════════════════════════════════════════════════════
...
// ─── Context Query (what the AI sees) ───────────────────────────────
...
impl SharedPentestState {
    ...
    pub fn ingest_result(&mut self, tool_name: &str, result: &serde_json::Value) {
        ...
        // Auto-extract discovered endpoints from recon tools
        match tool_name {
        ...
        if let Some(findings_arr) = result["findings"].as_array() {
            for f in findings_arr {
                self.findings.push(Finding {
                    ...
                    title: f["title"].as_str().or(f["template_name"].as_str()).unwrap_or("Unknown").to_string(),
        ...
        // Auto-extract tech fingerprints
    ...
    pub fn register_oast_payload(&mut self, payload_id: &str, injected_at: &str, injection_point: &str, tool_name: &str) {
        self.oast_correlations.insert(payload_id.to_string(), OastCorrelation {
    ...
    pub fn confirm_oast_callback(&mut self, payload_id: &str, details: serde_json::Value) {
        if let Some(corr) = self.oast_correlations.get_mut(payload_id) {
            ...
            // Auto-promote related findings to "certain" confidence
