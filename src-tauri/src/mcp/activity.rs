// ═══════════════════════════════════════════════════════════════════════
//  MCP Activity & Traffic Logging
// ═══════════════════════════════════════════════════════════════════════

use super::types::{ActivityEntry, McpTrafficEntry};

// ─── Activity Log Globals ───────────────────────────────────────────
...
pub fn tool_category(name: &str) -> &'static str {
    match name {
        "send_request" | "repeat_request" | "h2_send_request" | "mtls_send_request" | "send_to_repeater" => "http",
        "active_scan" | "scan_target" | "full_auto_scan" | "dom_invader" | "passive_scan" => "scanner",
        "crawl_target" | "discover_content" | "discover_subdomains" | "dns_resolve" | "find_secrets" | "analyze_target" | "h2_detect_support" | "js_link_finder" => "recon",
        "fuzz_request" | "custom_attack" | "race_request" | "timing_attack" | "send_to_intruder" => "intruder",
        "smuggling_send" | "detect_smuggling" | "raw_tcp_send" | "test_auth_bypass" | "test_open_redirect" => "exploit",
        "browser_navigate" | "browser_execute_js" | "session_from_browser" | "browser_network_traffic" => "browser",
        "encode" | "decode" | "hash" | "smart_decode" | "analyze_jwt" | "h2_translate" | "process_payload" | "generate_payload" => "codec",
        "proxy_start" | "proxy_stop" | "proxy_status" | "proxy_get_traffic" | "proxy_search_traffic"
            | "proxy_toggle_intercept" | "proxy_add_match_replace" | "proxy_get_match_replace"
            | "proxy_add_interception_rule" | "proxy_remove_interception_rule"
            | "proxy_remove_match_replace"
            | "proxy_add_tls_passthrough" | "proxy_set_upstream"
            | "proxy_get_websocket_messages" | "proxy_get_capabilities" | "proxy_get_statistics"
            | "proxy_clear_traffic" | "proxy_export_traffic"
            | "get_intercepted" | "forward_intercepted"
            | "proxy_annotate_traffic" => "proxy",
        "oast_generate_payload" | "oast_poll_interactions" | "oast_verify" | "oast_start_server" | "oast_start_dns_server" | "oast_start_smtp_server" | "collaborator_everywhere" => "oast",
        ...
        "crtsh_search" | "wayback_lookup" | "whois_lookup" | "asn_lookup" | "favicon_hash" | "reverse_ip_lookup" | "graphql_introspect" | "hackertarget_lookup" | "ip_geolocation" | "tech_detect" => "osint",
...
pub fn log_activity_start(tool_name: &str, params: &serde_json::Value) -> u64 {
    ...
    if let Ok(mut log) = ACTIVITY_LOG.lock() {
        log.push(entry);
        if log.len() > 300 { let drain = log.len() - 300; log.drain(..drain); }
    }
    id
}
...
pub fn log_mcp_traffic(entry: McpTrafficEntry) {
    if let Ok(mut log) = MCP_TRAFFIC_LOG.lock() {
        log.push(entry);
        if log.len() > 1000 { let drain = log.len() - 1000; log.drain(..drain); }
    }
}
