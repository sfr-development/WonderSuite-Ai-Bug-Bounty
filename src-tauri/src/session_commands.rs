use crate::session::{self, Cookie, SessionHandle, SessionMacro, MacroStep, MacroExtract, SessionRule, SessionScope, SessionAction};
...
pub async fn session_get_cookies(
    ...
    let cookies = if let Some(d) = domain {
        session.cookie_jar.cookies.iter()
            .filter(|c| c.domain.contains(&d))
            .cloned()
            .collect()
    } else {
...
#[tauri::command]
pub async fn session_clear_cookies(
    state: tauri::State<'_, SessionHandle>,
) -> Result<String, String> {
    let mut session = state.lock().await;
...
#[tauri::command]
pub async fn session_export_cookies(
    state: tauri::State<'_, SessionHandle>,
) -> Result<String, String> {
    let session = state.lock().await;
    serde_json::to_string_pretty(&session.cookie_jar.cookies)
...
#[tauri::command]
pub async fn session_get_macros(
    state: tauri::State<'_, SessionHandle>,
) -> Result<Vec<SessionMacro>, String> {
    let session = state.lock().await;
...
pub async fn session_run_macro(
    ...
    let session = state.lock().await;
    let the_macro = session.macros.iter()
...
// ── Session Rule Commands ───────────────────────────────────────────────────

#[tauri::command]
pub async fn session_get_rules(
    state: tauri::State<'_, SessionHandle>,
) -> Result<Vec<SessionRule>, String> {
    let session = state.lock().await;
...
pub async fn session_create_rule(
    ...
    let parsed_actions: Vec<SessionAction> = actions.into_iter().filter_map(|a| {
        ...
        Some(match action_type {
            ...
            "run_macro" => SessionAction::RunMacro {
                macro_id: a["macro_id"].as_str()?.to_string(),
            },
    ...
    let id = uuid::Uuid::new_v4().to_string();
    let mut session = state.lock().await;
    session.rules.push(SessionRule {
        id: id.clone(),
        name,
        enabled: true,
        scope,
