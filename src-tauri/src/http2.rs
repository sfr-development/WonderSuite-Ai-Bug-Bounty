...
pub async fn send_h2_request(req: &Http2Request) -> Result<Http2Response, String> {
    ...
    Ok(Http2Response { status, headers, body, stream_id: req.stream_id, protocol_version: "h2".into() })
}
...
pub fn describe_flags(frame_type: &Http2FrameType, flags: u8) -> Vec<String> {
    ...
    match frame_type {
        Http2FrameType::Data => {
            if flags & 0x01 != 0 {
                flag_names.push("END_STREAM".into());
            }
            if flags & 0x08 != 0 {
                flag_names.push("PADDED".into());
            }
        }
        Http2FrameType::Headers => {
            ...
            if flags & 0x04 != 0 {
                flag_names.push("END_HEADERS".into());
            }
            if flags & 0x08 != 0 {
                flag_names.push("PADDED".into());
            }
            if flags & 0x20 != 0 {
                flag_names.push("PRIORITY".into());
            }
        }
        Http2FrameType::Settings => {
            if flags & 0x01 != 0 {
                flag_names.push("ACK".into());
            }
        }
        Http2FrameType::Ping => {
            if flags & 0x01 != 0 {
