...
pub fn generate_html_report(title: &str, findings: &[ReportFinding], config: &ReportConfig) -> String {
    ...
    let filtered: Vec<&ReportFinding> = sorted
        .iter()
        .filter(|f| {
            if let Some(ref sevs) = config.severity_filter {
                if !sevs.contains(&f.severity) {
                    return false;
                }
            }
            if let Some(ref confs) = config.confidence_filter {
                if !confs.contains(&f.confidence) {
                    return false;
                }
            }
            true
    ...
    let findings_html: String = filtered
        .iter()
        .enumerate()
        .map(|(i, f)| {
            ...
            let evidence_section = if config.include_evidence {
                f.evidence
                    .as_ref()
                    .map(|e| {
                        format!("<div class='evidence'><h4>Evidence</h4><pre>{}</pre></div>", html_escape(e))
                    })
                    .unwrap_or_default()
            } else {
                String::new()
            ...
            let remediation_section = if config.include_remediation {
                f.remediation
                    ...
                    .map(|r| {
                        format!(
                            "<div class='remediation'><h4>Remediation</h4><p>{}</p></div>",
                            html_escape(r)
                        )
                    })
                    .unwrap_or_default()
            ...
            let param_info = f
                .parameter
                .as_ref()
                .map(|p| format!("<span class='param'>Parameter: {}</span>", html_escape(p)))
                .unwrap_or_default();
