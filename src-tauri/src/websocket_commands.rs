...
impl WsManager {
    pub fn new() -> Self {
        Self { connections: HashMap::new(), match_replace_rules: Vec::new(), next_msg_id: 1 }
    }
...
pub async fn ws_send_frame(
    ...
    for rule in &mgr.match_replace_rules {
        ...
        if rule.direction != "sent" && rule.direction != "both" {
            continue;
        }
        if rule.is_regex {
...
#[tauri::command]
pub async fn ws_list_connections(state: tauri::State<'_, WsState>) -> Result<Vec<serde_json::Value>, String> {
    let mgr = state.lock().await;
    Ok(mgr
        .connections
        .values()
...
#[tauri::command]
pub async fn ws_get_match_replace(state: tauri::State<'_, WsState>) -> Result<Vec<WsMatchReplace>, String> {
    let mgr = state.lock().await;
