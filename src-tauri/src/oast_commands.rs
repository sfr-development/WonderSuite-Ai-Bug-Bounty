...
use crate::oast::{self, OastInteraction, OastPayload};
...
pub async fn oast_generate(
    ...
    let desc = if let Some(vt) = &vuln_type { format!("[{}] {}", vt, description) } else { description };
...
#[tauri::command]
pub async fn oast_poll_interactions(correlation_id: Option<String>) -> Result<Vec<OastInteraction>, String> {
...
pub async fn oast_collaborator_everywhere(
    ...
    let mut store = get_payloads().lock().await;
    let out: Vec<serde_json::Value> = headers
        .into_iter()
        .map(|(header, value, oast_payload)| {
            store.push(oast_payload.clone());
            serde_json::json!({
                "header": header,
                "value": value,
