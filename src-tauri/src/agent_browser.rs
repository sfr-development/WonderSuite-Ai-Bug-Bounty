...
// ═══════════════════════════════════════════════════════════════════════════
//  Agent Browser — Stealth Anti-Detect Chrome with CDP Control
// ═══════════════════════════════════════════════════════════════════════════
...
fn next_id() -> u64 { CDP_CMD_ID.fetch_add(1, Ordering::Relaxed) }
...
async fn cdp_send_raw(
    ...
    sink.send(WsMessage::Text(cmd.to_string().into()))
        .await
    ...
    loop {
        ...
        if remaining.is_zero() { return Err("CDP timeout (15s)".into()); }
...
async fn cdp(state: &AgentBrowserState, method: &str, params: serde_json::Value) -> Result<serde_json::Value, String> {
    // Auto-reconnect if browser is alive but CDP dropped
    ...
    let sink = sink_guard.as_mut().ok_or("CDP not connected — launch the agent browser first with agent_browser_launch")?;
...
async fn cdp_eval(state: &AgentBrowserState, expr: &str) -> Result<serde_json::Value, String> {
    let r = cdp(state, "Runtime.evaluate", serde_json::json!({
...
fn bezier_points(x1: f64, y1: f64, x2: f64, y2: f64, steps: usize) -> Vec<(f64, f64)> {
    ...
    (0..=steps).map(|i| {
        ...
        let mt = 1.0 - t;
        let x = mt.powi(3)*x1 + 3.0*mt.powi(2)*t*cx1 + 3.0*mt*t.powi(2)*cx2 + t.powi(3)*x2;
        let y = mt.powi(3)*y1 + 3.0*mt.powi(2)*t*cy1 + 3.0*mt*t.powi(2)*cy2 + t.powi(3)*y2;
        (x, y)
    }).collect()
}
...
fn typing_delay() -> u64 {
    let mut rng = rand::thread_rng();
    if rng.gen_range(0..10) == 0 { rng.gen_range(200..500) } else { rng.gen_range(50..180) }
}
...
pub async fn agent_browser_launch(
    ...
    let mut args: Vec<String> = vec![
        ...
        "--flag-switches-begin".into(), "--flag-switches-end".into(),
        ...
        "--window-size=1920,1080".into(), "--start-maximized".into(),
        "--ignore-certificate-errors".into(), "--allow-insecure-localhost".into(),
        "--disable-extensions".into(),
    ];
    if is_headless { args.push("--headless=new".into()); }
    if let Some(ref ua) = user_agent { args.push(format!("--user-agent={}", ua)); }
    args.push("about:blank".into());

    #[cfg(target_os = "windows")]
    let child = {
        use std::os::windows::process::CommandExt;
        std::process::Command::new(&browser.path).args(&args).creation_flags(0x08000000)
            .spawn().map_err(|e| format!("Launch failed: {}", e))?
    };
    #[cfg(not(target_os = "windows"))]
    let child = std::process::Command::new(&browser.path).args(&args)
        .spawn().map_err(|e| format!("Launch failed: {}", e))?;
                            ...
                            if let Some(ws_url) = page.get("webSocketDebuggerUrl").and_then(|v| v.as_str()) {
                                if let Ok((ws, _)) = tokio_tungstenite::connect_async(ws_url).await {
                                    ...
                                    *state.stream.lock().await = Some(stream);
                                    ...
                                    // Enable domains + inject stealth (use raw to avoid recursion)
                                    {
                                        let mut s = state.sink.lock().await;
                                        let mut r = state.stream.lock().await;
                                        if let (Some(sk), Some(st)) = (s.as_mut(), r.as_mut()) {
                                            let _ = cdp_send_raw(sk, st, "Page.enable", serde_json::json!({})).await;
                                            let _ = cdp_send_raw(sk, st, "Network.enable", serde_json::json!({})).await;
                                            let _ = cdp_send_raw(sk, st, "DOM.enable", serde_json::json!({})).await;
                                            let _ = cdp_send_raw(sk, st, "Runtime.enable", serde_json::json!({})).await;
                                            let _ = cdp_send_raw(sk, st, "Page.addScriptToEvaluateOnNewDocument",
                                                serde_json::json!({ "source": stealth_js() })).await;
...
pub async fn agent_browser_close(state: tauri::State<'_, AgentBrowserState>) -> Result<String, String> {
    if !state.running.load(Ordering::Relaxed) { return Ok("Not running".into()); }
...
#[tauri::command]
pub async fn agent_browser_status(state: tauri::State<'_, AgentBrowserState>) -> Result<serde_json::Value, String> {
    Ok(serde_json::json!({
...
#[tauri::command]
pub async fn agent_browser_navigate(state: tauri::State<'_, AgentBrowserState>, url: String) -> Result<serde_json::Value, String> {
    // Auto-reconnect CDP if browser is running but connection was lost
    if state.sink.lock().await.is_none() && state.running.load(Ordering::Relaxed) {
        connect_cdp(&state).await;
    ...
    *state.current_title.lock().await = cur_title.clone();
    Ok(serde_json::json!({ "success": true, "url": cur_url, "title": cur_title, "frame_id": result.get("frameId") }))
}
...
pub async fn agent_browser_get_content(state: tauri::State<'_, AgentBrowserState>) -> Result<String, String> {
    ...
    if html.len() > 50000 { Ok(format!("{}…(truncated {})", &html[..50000], html.len())) } else { Ok(html) }
}
...
pub async fn agent_browser_get_text(state: tauri::State<'_, AgentBrowserState>) -> Result<String, String> {
    ...
    if text.len() > 30000 { Ok(format!("{}…(truncated)", &text[..30000])) } else { Ok(text) }
}

#[tauri::command]
pub async fn agent_browser_query_selector(state: tauri::State<'_, AgentBrowserState>, selector: String) -> Result<serde_json::Value, String> {
    let js = format!(r#"(()=>{{const el=document.querySelector('{}');if(!el)return null;const r=el.getBoundingClientRect();return{{tag:el.tagName.toLowerCase(),id:el.id||null,className:el.className||null,text:(el.innerText||'').slice(0,500),href:el.href||null,value:el.value||null,type:el.type||null,name:el.name||null,x:r.x+r.width/2,y:r.y+r.height/2,width:r.width,height:r.height,visible:r.width>0&&r.height>0}}}})()"#, selector.replace('\'', "\\'"));
    cdp_eval(&state, &js).await
}

#[tauri::command]
pub async fn agent_browser_query_selector_all(state: tauri::State<'_, AgentBrowserState>, selector: String) -> Result<serde_json::Value, String> {
    let js = format!(r#"(()=>{{const els=document.querySelectorAll('{}');return Array.from(els).slice(0,100).map(el=>{{const r=el.getBoundingClientRect();return{{tag:el.tagName.toLowerCase(),id:el.id||null,text:(el.innerText||'').slice(0,200),href:el.href||null,value:el.value||null,name:el.name||null,x:r.x+r.width/2,y:r.y+r.height/2}}}})}})()"#, selector.replace('\'', "\\'"));
    cdp_eval(&state, &js).await
}

#[tauri::command]
pub async fn agent_browser_get_links(state: tauri::State<'_, AgentBrowserState>) -> Result<serde_json::Value, String> {
...
#[tauri::command]
pub async fn agent_browser_get_forms(state: tauri::State<'_, AgentBrowserState>) -> Result<serde_json::Value, String> {
...
#[tauri::command]
pub async fn agent_browser_get_inputs(state: tauri::State<'_, AgentBrowserState>) -> Result<serde_json::Value, String> {
...
#[tauri::command]
pub async fn agent_browser_click(state: tauri::State<'_, AgentBrowserState>, selector: String) -> Result<String, String> {
    // Get element position via JS
    let js = format!(r#"(()=>{{const el=document.querySelector('{}');if(!el)return null;el.scrollIntoView({{block:'center'}});const r=el.getBoundingClientRect();return{{x:r.x+r.width/2,y:r.y+r.height/2}}}})()"#, selector.replace('\'', "\\'"));
    let pos = cdp_eval(&state, &js).await?;
    if pos.is_null() { return Err(format!("Element not found: {}", selector)); }
    let x = pos.get("x").and_then(|v| v.as_f64()).ok_or("No x")?;
    let y = pos.get("y").and_then(|v| v.as_f64()).ok_or("No y")?;

    // Pre-compute all random values before async (rng is !Send)
...

    {
        ...
        if let Some(sink) = sink_guard.as_mut() {
            for (i, (px, py)) in points.iter().enumerate() {
                let _ = cdp_fire(sink, "Input.dispatchMouseEvent", serde_json::json!({
    ...
    // Click
    cdp(&state, "Input.dispatchMouseEvent", serde_json::json!({
...
#[tauri::command]
pub async fn agent_browser_type(state: tauri::State<'_, AgentBrowserState>, selector: String, text: String) -> Result<String, String> {
    // Focus by clicking
    ...
    // Pre-compute delays (rng is !Send)
    let delays: Vec<u64> = {
        let mut rng = rand::thread_rng();
        text.chars().map(|_| if rng.gen_range(0..10) == 0 { rng.gen_range(200..500) } else { rng.gen_range(50..180) }).collect()
    };

    // Type each char
    for (i, ch) in text.chars().enumerate() {
        let key_text = ch.to_string();
        cdp(&state, "Input.dispatchKeyEvent", serde_json::json!({
...
#[tauri::command]
pub async fn agent_browser_press_key(state: tauri::State<'_, AgentBrowserState>, key: String) -> Result<String, String> {
    cdp(&state, "Input.dispatchKeyEvent", serde_json::json!({ "type": "keyDown", "key": key, "code": key })).await?;
    tokio::time::sleep(tokio::time::Duration::from_millis(50)).await;
    cdp(&state, "Input.dispatchKeyEvent", serde_json::json!({ "type": "keyUp", "key": key, "code": key })).await?;
    Ok(format!("Pressed {}", key))
}

#[tauri::command]
pub async fn agent_browser_scroll(state: tauri::State<'_, AgentBrowserState>, direction: String, amount: Option<i64>) -> Result<String, String> {
    let d = amount.unwrap_or(300);
    let (dx, dy) = match direction.as_str() { "up" => (0,-d), "left" => (-d,0), "right" => (d,0), _ => (0,d) };
    cdp(&state, "Input.dispatchMouseEvent", serde_json::json!({
...
#[tauri::command]
pub async fn agent_browser_select_option(state: tauri::State<'_, AgentBrowserState>, selector: String, value: String) -> Result<String, String> {
    let js = format!(r#"(()=>{{const el=document.querySelector('{}');if(!el)return'not found';el.value='{}';el.dispatchEvent(new Event('change',{{bubbles:true}}));return'selected: '+el.value}})()"#, selector.replace('\'', "\\'"), value.replace('\'', "\\'"));
    cdp_eval_str(&state, &js).await
}

#[tauri::command]
pub async fn agent_browser_fill_form(state: tauri::State<'_, AgentBrowserState>, fields: HashMap<String, String>) -> Result<serde_json::Value, String> {
...
#[tauri::command]
pub async fn agent_browser_clear_field(state: tauri::State<'_, AgentBrowserState>, selector: String) -> Result<String, String> {
    let js = format!(r#"(()=>{{const el=document.querySelector('{}');if(!el)return false;el.focus();el.value='';el.dispatchEvent(new Event('input',{{bubbles:true}}));return true}})()"#, selector.replace('\'', "\\'"));
    cdp_eval(&state, &js).await?;
...
#[tauri::command]
pub async fn agent_browser_screenshot(state: tauri::State<'_, AgentBrowserState>, full_page: Option<bool>) -> Result<String, String> {
...
#[tauri::command]
pub async fn agent_browser_screenshot_element(state: tauri::State<'_, AgentBrowserState>, selector: String) -> Result<String, String> {
    let js = format!(r#"(()=>{{const el=document.querySelector('{}');if(!el)return null;const r=el.getBoundingClientRect();return{{x:r.x,y:r.y,width:r.width,height:r.height}}}})()"#, selector.replace('\'', "\\'"));
    let rect = cdp_eval(&state, &js).await?;
    if rect.is_null() { return Err("Element not found".into()); }
...
#[tauri::command]
pub async fn agent_browser_set_viewport(state: tauri::State<'_, AgentBrowserState>, width: u32, height: u32, device_scale: Option<f64>, mobile: Option<bool>) -> Result<String, String> {
...
#[tauri::command]
pub async fn agent_browser_evaluate(state: tauri::State<'_, AgentBrowserState>, expression: String) -> Result<serde_json::Value, String> {
    cdp_eval(&state, &expression).await
}

#[tauri::command]
pub async fn agent_browser_evaluate_on_new_doc(state: tauri::State<'_, AgentBrowserState>, script: String) -> Result<String, String> {
    let r = cdp(&state, "Page.addScriptToEvaluateOnNewDocument", serde_json::json!({ "source": script })).await?;
...
#[tauri::command]
pub async fn agent_browser_new_tab(state: tauri::State<'_, AgentBrowserState>, url: Option<String>) -> Result<serde_json::Value, String> {
    cdp(&state, "Target.createTarget", serde_json::json!({ "url": url.unwrap_or("about:blank".into()) })).await
}

#[tauri::command]
pub async fn agent_browser_list_tabs(state: tauri::State<'_, AgentBrowserState>) -> Result<serde_json::Value, String> {
    let cdp_port = *state.cdp_port.lock().await;
    let client = reqwest::Client::builder().danger_accept_invalid_certs(true).timeout(std::time::Duration::from_secs(3)).build().map_err(|e| e.to_string())?;
    let resp = client.get(format!("http://127.0.0.1:{}/json", cdp_port)).send().await.map_err(|e| e.to_string())?;
    let pages: serde_json::Value = resp.json().await.map_err(|e| e.to_string())?;
    Ok(pages)
}

#[tauri::command]
pub async fn agent_browser_close_tab(state: tauri::State<'_, AgentBrowserState>, target_id: String) -> Result<String, String> {
    cdp(&state, "Target.closeTarget", serde_json::json!({ "targetId": target_id })).await?;
    Ok(format!("Closed {}", target_id))
}

#[tauri::command]
pub async fn agent_browser_switch_tab(state: tauri::State<'_, AgentBrowserState>, target_id: String) -> Result<String, String> {
...
#[tauri::command]
pub async fn agent_browser_get_cookies(state: tauri::State<'_, AgentBrowserState>) -> Result<serde_json::Value, String> {
    let r = cdp(&state, "Network.getCookies", serde_json::json!({})).await?;
    Ok(r.get("cookies").cloned().unwrap_or(serde_json::json!([])))
}

#[tauri::command]
pub async fn agent_browser_set_cookie(state: tauri::State<'_, AgentBrowserState>, name: String, value: String, domain: String, path: Option<String>) -> Result<String, String> {
...
#[tauri::command]
pub async fn agent_browser_delete_cookie(state: tauri::State<'_, AgentBrowserState>, name: String, domain: String) -> Result<String, String> {
    cdp(&state, "Network.deleteCookies", serde_json::json!({ "name": name, "domain": domain })).await?;
    Ok(format!("Cookie {} deleted", name))
}

#[tauri::command]
pub async fn agent_browser_clear_all_cookies(state: tauri::State<'_, AgentBrowserState>) -> Result<String, String> {
    cdp(&state, "Network.clearBrowserCookies", serde_json::json!({})).await?;
    Ok("All cookies cleared".into())
}

#[tauri::command]
pub async fn agent_browser_get_local_storage(state: tauri::State<'_, AgentBrowserState>) -> Result<serde_json::Value, String> {
...
#[tauri::command]
pub async fn agent_browser_set_local_storage(state: tauri::State<'_, AgentBrowserState>, key: String, value: String) -> Result<String, String> {
...
#[tauri::command]
pub async fn agent_browser_wait_for_element(state: tauri::State<'_, AgentBrowserState>, selector: String, timeout_ms: Option<u64>) -> Result<bool, String> {
    let deadline = tokio::time::Instant::now() + tokio::time::Duration::from_millis(timeout_ms.unwrap_or(10000));
    loop {
        if tokio::time::Instant::now() >= deadline { return Ok(false); }
        let r = agent_browser_query_selector(state.clone(), selector.clone()).await;
        if let Ok(v) = &r { if !v.is_null() { return Ok(true); } }
        tokio::time::sleep(tokio::time::Duration::from_millis(300)).await;
    }
}

#[tauri::command]
pub async fn agent_browser_wait_for_navigation(state: tauri::State<'_, AgentBrowserState>, timeout_ms: Option<u64>) -> Result<String, String> {
...
// ═══════════════════════════════════════════════════════════════════════════
//  Network & Stealth Config
// ═══════════════════════════════════════════════════════════════════════════

#[tauri::command]
pub async fn agent_browser_set_extra_headers(state: tauri::State<'_, AgentBrowserState>, headers: HashMap<String, String>) -> Result<String, String> {
    cdp(&state, "Network.setExtraHTTPHeaders", serde_json::json!({ "headers": headers })).await?;
    Ok(format!("{} headers set", headers.len()))
}

#[tauri::command]
pub async fn agent_browser_block_urls(state: tauri::State<'_, AgentBrowserState>, patterns: Vec<String>) -> Result<String, String> {
    cdp(&state, "Network.setBlockedURLs", serde_json::json!({ "urls": patterns })).await?;
    Ok(format!("Blocking {} patterns", patterns.len()))
}

#[tauri::command]
pub async fn agent_browser_set_user_agent(state: tauri::State<'_, AgentBrowserState>, user_agent: String) -> Result<String, String> {
    cdp(&state, "Network.setUserAgentOverride", serde_json::json!({ "userAgent": user_agent })).await?;
    Ok(format!("UA: {}", user_agent))
}

#[tauri::command]
pub async fn agent_browser_set_geolocation(state: tauri::State<'_, AgentBrowserState>, latitude: f64, longitude: f64, accuracy: Option<f64>) -> Result<String, String> {
    cdp(&state, "Emulation.setGeolocationOverride", serde_json::json!({
...
#[tauri::command]
pub async fn agent_browser_set_timezone(state: tauri::State<'_, AgentBrowserState>, timezone_id: String) -> Result<String, String> {
    cdp(&state, "Emulation.setTimezoneOverride", serde_json::json!({ "timezoneId": timezone_id })).await?;
    Ok(format!("TZ: {}", timezone_id))
}

#[tauri::command]
pub async fn agent_browser_handle_dialog(state: tauri::State<'_, AgentBrowserState>, accept: bool, prompt_text: Option<String>) -> Result<String, String> {
    let mut p = serde_json::json!({ "accept": accept });
    if let Some(t) = prompt_text { p["promptText"] = serde_json::json!(t); }
