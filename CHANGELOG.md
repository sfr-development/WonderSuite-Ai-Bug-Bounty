...
### Added
- Integrated **HttpViewer** Premium real-time syntax highlighter for HTTP Request/Response, JSON, XML, HTML, URL-encoded parameter forms, and HEX dumps across Repeater (Replay), Intruder (Attack results), Sitemap, and Proxy traffic history details.
- Real-time editable syntax highlighting in the Repeater request panel via synchronized scroll textarea overlay.
- Dynamic key-value color highlighting in the parsed Headers tabs in Proxy history and Sitemap.
- Optional silent background sending in Repeater to allow seamless non-intrusive workspace transfers.
- Stufenlose continuous flex split panel resizing in Repeater and Intruder views for modern wide display adaptability.

### Fixed
- Fixed silent proxy bypasses in Desktop Active Scanner, MCP Active Scanner, Intruder, and Repeater by automatically routing requests through the local loopback proxy when active.
- Prevented duplicate scan request logging in Proxy history and Sitemap.
- Guaranteed zero target-side internal header leakage by removing internal source headers at MITM proxy outbound forwarding stage.
- Resolved Windows static compilation dependencies by integrating a portable NASM downloader.
