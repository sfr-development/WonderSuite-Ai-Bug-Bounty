...
The v0.3.21 release commit (`7f4767e`) was botched: an older snapshot of
the repo got committed on top of v0.3.20 with only the version bumped.
The net diff was **+1816 / −35969 lines across 139 files** — the Port
Scanner module, WinDivert bundling, the bot-detection screenshot,
preview GIF + MP4, large parts of the CHANGELOG, ~220 lines of the
README, and ~50 other files were silently rolled back. On top of that,
`createUpdaterArtifacts` flipped from `true` to `false` in
`tauri.conf.json`, so the v0.3.21 GitHub release shipped without
`latest.json` and without signed `.sig` artifacts. That is why no
existing install ever saw the v0.3.21 update modal — the auto-updater
endpoint returned 404.

v0.3.22 is an identical content-snapshot of v0.3.20 with only the
version metadata advanced to 0.3.22 so the auto-updater can finally
move existing installs forward again. Apologies to everyone who saw
the broken v0.3.21 release on GitHub — please update to v0.3.22 (or
ignore v0.3.21 entirely).

### Note on the lost v0.3.21 feature work
The commit title of `7f4767e` claimed "Premium HTTP Syntax Highlighting
and Unified Proxy Loopback Routing". A `HttpViewer.tsx` component plus
`x-wondersuite-source` header plumbing in the proxy / scanner / intruder
were in fact in that commit, but they were tangled with the regression
and have **not** been carried into v0.3.22. They will be re-introduced
in a follow-up release on a clean baseline.
